...
2. Publish and document the standalone skill (`skill/SKILL.md`) via `docs/skill-usage.md`, which already defines how it operates agnostically via `.atcr/reviews/<id>/` — confirm coverage and cite it here rather than re-deriving it.
3. Add a repo-local end-to-end test/check proving the `--output-dir` + reconcile contract (documented in `docs/code-review-backend.md`) that private-skill consumers depend on remains stable. This does not touch the private skills themselves — Epic 12.0 already validated that integration end-to-end from the external (`claude-prompts`) side.
4. Write `install.sh`, the one confirmed gap (no install script exists anywhere in the repo today). Document that `atcr doctor` (self-test) and `atcr quickstart` (onboarding wizard) already satisfy the self-test / quick-start requirement without depending on private APIs — do not build new commands.
...
## Refinements (2026-07-03) — Addendum: public skill packaging convention

Decided while scoping Epic 20.1 (Public TD Resolve Skill), the first new public capability proposed since this epic shipped `skill/SKILL.md`: new public capabilities under `skill/` ship as **separate, single-capability skill packages** (e.g. `skill/atcr-resolve/SKILL.md` alongside the existing `skill/SKILL.md`), not as one dispatcher skill routing sub-commands to per-command files.

**Rationale:**
- Claude Code skills are discovered by matching a user's intent against each skill's `description` frontmatter. One skill covering multiple distinct capabilities (review vs. resolve) dilutes that match quality.
- The private `claude-prompts` skillset already follows one-skill-per-capability (9+ skills, each its own directory + `instructions.md`) — matching that convention keeps the public and private skill sets structurally consistent, and is a pattern already proven at scale rather than a new one invented for the public side.
- Shared boilerplate (binary-on-`PATH` check, `.atcr/` path-safety rules, prerequisites — currently embedded directly in `skill/SKILL.md`'s Prerequisites section) should move to a shared reference doc (e.g. `skill/CONVENTIONS.md`) that each skill's `SKILL.md` points to, rather than being duplicated per skill or forcing a single dispatcher skill just to avoid duplication.

**Applies to:** Epic 20.1 and any future public skill added under `skill/`. Not a change requested to `skill/SKILL.md` itself by this epic — the shared-conventions extraction happens whenever a second public skill is actually added (i.e., as part of Epic 20.1's execution, not retroactively here).
