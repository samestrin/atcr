# Epic Plan 7.0: Persona Ecosystem

**Estimated Durations**: 4 weeks initial; ongoing community curation
...
Epic 3.4 (per-run scorecard) adds corroboration tracking per persona, making persona quality measurable. This epic builds on that: community personas can earn leaderboard scores, making quality visible and gamified.
...
Integrate with Epic 3.4's scorecard: each installed persona accumulates a corroboration score over time. `atcr personas list --scores` shows which personas are finding the most corroborated issues on the user's actual codebase.
...
- [ ] `atcr personas list` shows installed personas with name, namespace, version, and (if 3.4 data available) corroboration rate.
...
6. **Per-persona corroboration integration**: wire 3.4 scorecard into `atcr personas list --scores`.
...
- Epic 3.4 (per-persona scorecard) — optional enrichment for Phase 3; personas work without it.
