...
- ⏸️ **Path correction for Community Personas:** Plan lists `personas/` for the new YAML and prompt template, but Epic 19.6 moved community personas to `personas/community/`. Suggested action: update the Components Touched and Proposed Solution to explicitly use `personas/community/` so the agent places the files in the correct location.
