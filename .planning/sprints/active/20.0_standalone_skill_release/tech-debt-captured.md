...
## TD-003 — README command table is missing 7 registered commands (LOW)
**Origin:** Phase 3, task 3.1 Story 4 doc-accuracy verification, 2026-07-11
**File:** README.md:77-93
**Issue:** The README `## Commands` table lists 15 of the 22 commands registered in `newRootCmd` (cmd/atcr/main.go:185-208). Missing: `github`, `scorecard`, `leaderboard`, `benchmark`, `personas`, `models`, `version`. The listed commands are accurate; the table is just incomplete. This is a pre-existing gap (not introduced by the Story 1 dispatcher rewrite), so it is out of Story 4's targeted post-dispatcher accuracy scope. `skill/SKILL.md`'s routing table already lists all 22 (verified by `skill/skill_test.go`), so the authoritative command surface is correct.
**Why accepted:** Story 4 corrects only dispatcher-introduced inaccuracies with no restructuring; completing the README table is additive authoring better done by Epic 33.0's comprehensive documentation sweep, which explicitly audits "all CLI commands ... against the final production codebase."
**Fix in:** Epic 33.0 (Final Code Review + Documentation Sweep) — complete the README command table against `newRootCmd` (mirror `skill/SKILL.md`'s one-line descriptions), or add a "see `atcr --help` for the full list" note if a curated subset is intended.
