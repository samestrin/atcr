...
**Resolved:** 2026-07-14 — added `sarif` to reportInputSchema's enum (tools.go:230), the ReportArgs/descReport doc strings, and updated tools_test.go:69,111; TestReportHandler_SarifParity now also asserts transport-level (`callOK`) parity. MCP clients can request `--format=sarif` over the wire.
