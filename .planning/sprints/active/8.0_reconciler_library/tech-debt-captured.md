...
## TD-007 — Local pre-push hook does not exercise the ./reconcile module (LOW)
**Origin:** Phase 5, task 5.2 GREEN, 2026-06-23
**File:** .githooks/pre-push
**Issue:** `.githooks/pre-push` mirrors root CI (`golangci-lint run` + `go test -race ./...`) but, like root `go test ./...`, does NOT cross the nested `./reconcile` `go.mod` boundary — so a `./reconcile` regression passes pre-push locally even though the new `ci.yml` `reconcile-module` job catches it at PR time. AC 06-02 Edge Case references the local hook mirror as something that "should also exercise ./reconcile so the same gap does not reopen locally," but task 5.2's Files list deliberately scopes `.githooks` out (it lists the hook as "reference," not "modify").
**Why accepted:** The CI boundary gap (the actual AC#7/#8 deliverable) is closed by `reconcile-module.yml` (tag push) + the `ci.yml` PR-time `reconcile-module` job; both proven locally by the deliberate-break simulation. The local hook is a developer-convenience mirror, not a required gate, and modifying it is out of task 5.2's explicit scope.
**Fix in:** a follow-on `resolve-td` session — add `(cd reconcile && go test ./...)` to `.githooks/pre-push` (and mirror in the tracked `.githooks/` per git-hooks-mirror-ci) so the local mirror matches the new `ci.yml` `reconcile-module` job and the boundary gap cannot reopen locally.
