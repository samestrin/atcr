...
## TD-008 — Non-boolean registry bool fields produce generic (not field-named) decode errors (LOW)
**Origin:** Phase 4, task 4.2.A adversarial review, 2026-06-13
**File:** internal/registry/config.go
**Issue:** A non-boolean YAML value for `supports_function_calling` (e.g. `"yes"`, `1`) is rejected by the strict YAML decoder as a generic type error rather than the field-named message AC 04-01 Error Scenario 2 sketches ("agent '%s': supports_function_calling must be a boolean, got %T"). This is safe — the strict decoder rejects it before any review starts, so there is no silent full-access grant — and it is identical to how the pre-existing `tools` and `role` reserved bool/enum fields behave. Purely an error-ergonomics gap, not a degradation defect.
**Why accepted:** No security or correctness impact (invalid values are rejected at load). Producing the exact field-named message requires bespoke per-field decoding (unmarshal into interface{} then type-check) that none of the existing reserved fields use; adding it only for `supports_function_calling` would be inconsistent, and adding it registry-wide is out of Story 4 scope.
**Fix in:** A registry error-ergonomics pass — add field-named decode-error hints for all bool/enum agent fields uniformly (mirroring `amendWithSettingsHint`), so non-boolean `tools`/`supports_function_calling` and invalid `role` values all name the offending agent + field.
