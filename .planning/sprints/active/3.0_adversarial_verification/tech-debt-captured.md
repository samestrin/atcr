...
## TD-015 — Unknown-severity findings skipped in the summary grid but counted in the total (LOW)
**Origin:** Phase 5, task 5.2.A adversarial review, 2026-06-14
**File:** internal/report/render.go:166-184 (writeSummaryGrid)
**Issue:** `writeSummaryGrid` iterates only the known severities (CRITICAL/HIGH/MEDIUM/LOW) and `continue`-skips any finding whose `Severity` is outside that set (e.g. a stray "INFO"). Such a finding is still counted in "Total findings" and still rendered with its own `### INFO` body header, so the grid's per-row sum can be less than the stated total — the grid silently under-represents unknown severities. The Phase 5 VERIFIED column inherits the same skip.
**Why accepted:** Pre-existing v1 behavior — the grid already skipped unknown severities before Phase 5; this is not a regression introduced by the verification work. The reconciler emits only canonical severities (the parser's extraction regex is `^(CRITICAL|HIGH|MEDIUM|LOW)\|`), so an unknown severity only reaches the renderer via hand-edited or third-party findings.json. Display-only inconsistency; the gate and counts elsewhere are unaffected.
**Fix in:** Report follow-up — add an "other" severity bucket/row (or fold unknown severities into LOW) so the grid's per-row sum always reconciles with "Total findings"; add a fixture with a non-canonical severity.

## TD-016 — Reviewer names containing a comma render ambiguously in the report (LOW)
**Origin:** Phase 5, task 5.2.A adversarial review, 2026-06-14
**File:** internal/report/render.go:313-318 (joinReviewers)
**Issue:** `joinReviewers` joins reviewer names with `", "` and the whole string is `esc()`'d at the call site. A reviewer name that itself contains a comma renders ambiguously — a reader cannot tell where one name ends and the next begins. It is NOT an injection (escaping is applied to the joined result, so HTML/newlines are neutralized); purely a display-correctness ambiguity.
**Why accepted:** Pre-existing v1 behavior (the join+esc predates Phase 5). Agent/reviewer names are registry keys constrained to identifier-like tokens, so a comma in a name does not occur in practice. Cosmetic only.
**Fix in:** Report follow-up — escape/join reviewer names per-element (or use a non-comma delimiter) if comma-bearing names ever become legitimate; otherwise document the no-comma assumption in `joinReviewers`.
