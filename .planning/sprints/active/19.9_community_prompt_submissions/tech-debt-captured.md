...
## TD-006 — Submission marker's `FixturePassed` is stamped `true` unconditionally (LOW)
**Origin:** Phase 3, task 3.5 integration gate review, 2026-07-10
**File:** internal/personas/submit.go:110
**Issue:** `Submit` hard-codes `SubmissionStatus.FixturePassed: true`; it never runs the fixture gate itself (that is `SubmitGate`, invoked separately in the cobra RunE at cmd/atcr/personas.go before `Submit`). The field can therefore never be `false`, yet it is documented (submissions.go) as "the local fixture gate passed before submission" — a real signal. A direct caller of exported `Submit` that skipped `SubmitGate` would stamp a false `true`.
**Why accepted:** The field is AC-02/03-02-mandated (the marker schema MUST carry a fixture-pass flag), and at the sole production call site (`personasSubmitContinuation`) `Submit` is only ever reached AFTER `SubmitGate` passes, so the value reflects a real invariant of the submit flow. Threading a bool through `Submit`'s signature for a value that is always `true` at the one call site is over-engineering ahead of any second caller. No `submitted`-status UI consumes the flag yet (Epic 30.0 owns the quality signal).
**Fix in:** future sprint / when a second `Submit` caller or a `list --status` surface appears — thread the actual `SubmitGate` outcome (or a typed "gate passed" token) into `Submit` so the marker reflects a verified result rather than a constant, and reconcile with AC 03-02's fixture-pass-flag requirement.

## TD-007 — `personas-authoring.md` §5 "maps 1:1" claim omits the `binding` index field (LOW)
**Origin:** Phase 4, task 4.3 docs-accuracy adversarial review, 2026-07-10
**File:** docs/personas-authoring.md:184
**Issue:** §5 ("The community index entry") states the JSON keys "map 1:1 to `PersonaIndexEntry` in `internal/personas/search.go`," but the shown JSON shape and field table (docs/personas-authoring.md:187-208) omit the `binding` field, which is a real struct member (`search.go:26`, `Binding string yaml/json:"binding,omitempty"`). The "1:1" claim is therefore loose. This is pre-existing §5 content NOT modified by Phase 4 (the new §8 graduation section correctly lists `binding` as optional); `binding` is also documented in §6, so reader impact is minimal.
**Why accepted:** Pre-existing content outside Phase 4's changed sections (§4 note, §7, §8); AC 05-01 DoD forbids changing other section content, and no Phase 4 AC covers §5. Surfaced by the 4.3 adversarial reviewer against the shipped struct; captured rather than fixed to stay within the docs-phase scope.
**Fix in:** future docs pass — either add a `binding` (optional) row to the §5 table, or soften "map 1:1" to note only the required + `tasks`/`tags` keys are shown and `binding` is documented in §6.
