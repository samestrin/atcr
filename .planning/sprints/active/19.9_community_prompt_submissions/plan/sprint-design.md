...
| GitHub auth/credential handling | `gh auth status` precondition check | Accidental logging of a token or auth header in error output | Never log the raw output of `gh auth status`/`gh.ExecContext`; surface only a boolean pass/fail plus a generic actionable message |
...
| `gh` fork/push/PR-create sequence | Single submission per invocation, network-bound against GitHub | Bounded by GitHub API latency; must not hang indefinitely on a stalled network call | Pass `context.Context` with a reasonable timeout into every `gh.ExecContext` call so a stalled network doesn't hang the CLI forever |
