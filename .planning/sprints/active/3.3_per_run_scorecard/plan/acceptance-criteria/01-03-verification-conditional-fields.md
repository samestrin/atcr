...
Scorecard records include `findings_verified`, `findings_refuted`, and `survived_skeptic_rate` only when a valid `reconciled/verification.json` is present; otherwise these keys are omitted from the JSON output.
...
## Design References
- [Adversarial Verification Interface](../../../../specifications/design-concepts/adversarial-verification-interface.md) — defines the `atcr verify` CLI and `verification.json` semantics that drive the conditional fields.

### Related Files (from codebase-discovery.json)
- `internal/scorecard/scorecard.go` — modify: conditional fields with `omitempty`
- `internal/reconcile/emit.go:40` — reference: `Verification` struct and `ReadReconciledFindings`
- `internal/verify/emit_verification.go` — reference: `VerificationFile` schema and `ReadVerificationResults` loader
...
- **Then** record contains `findings_verified: 8`, `findings_refuted: 2`, `survived_skeptic_rate: 0.80` (8 verified / (8 verified + 2 refuted))
