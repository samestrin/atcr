...
For every reconcile run, an aggregate record summarizing the full run is appended to the monthly JSONL file immediately after the per-reviewer records.
...
- `internal/scorecard/scorecard.go` — modify: aggregate record builder (sum findings, average rate, total cost)
...
- `internal/reconcile/reconcile.go:55` — reference: `Result{Findings, Summary}` used to derive reviewer counts
