...
## TD-024 — adjudication.json not bound to a baseline + lenient ambiguous.json decode (MEDIUM)
**Origin:** Phase 4, task 4.29 adversarial review, 2026-06-11
**File:** internal/reconcile/gate.go:81, internal/reconcile/ambiguous.go:115 (AmbiguousIDsFromFile)
**Issue:** (1) adjudication.json persists in reconciled/ and is re-applied on every reconcile run; it is keyed only by content-addressed cluster ids, with no binding to the ambiguous.json generation it was authored against — a stale decisions file from a prior session whose ids still match would re-merge silently. (2) AmbiguousIDsFromFile uses plain json.Unmarshal (not the strict DisallowUnknownFields decoder LoadAdjudication uses), and treats an empty/`null` ambiguous.json as a zero-id set, so an adjudication.json present with no ambiguous baseline fails opaquely as "unknown cluster id" instead of a clear "nothing to adjudicate".
**Why accepted:** Idempotency is fixed (validate against the preserved ambiguous.original.json; never clobber it), so repeated runs are deterministic and safe; the remaining risk is a cross-session stale file, which requires an operator to leave an unrelated adjudication.json in the review dir — an unusual, self-inflicted state. ambiguous.json is atcr-generated (not user-authored), so lenient decode is low-risk.
**Fix in:** v2 — record the baseline ambiguous.json content hash in adjudication.json and refuse to apply a decisions file whose baseline doesn't match; add strict decode + an explicit "no clusters to adjudicate" error.

## TD-025 — atcr_reconcile emits empty artifacts before the no-sources guard; empty base/head on empty range (LOW)
**Origin:** Phase 4, task 4.34 gate review, 2026-06-11
**File:** internal/mcp/handlers.go:170 (handleReconcile), internal/mcp/handlers.go:57 (handleRange)
**Issue:** (1) handleReconcile calls RunReconcile (which runs Reconcile+Emit) before the `len(SourcesScanned)==0` guard, so a review whose sources/ exists but holds no findings files gets empty reconciled artifacts written to disk as a side effect, then the handler returns the "no agent results" error — whereas the CLI rejects earlier and writes nothing. Both error; only the on-disk side effect differs. (2) handleRange echoes the raw input base/head on ErrEmptyRange, which are empty strings in the no-arg auto-detect case, so the zero-count "nothing to review" result carries no resolved-ref provenance.
**Why accepted:** Both are error/edge paths with no functional impact: the reconcile guard still errors (no false success), and the empty-range range result still honors the documented zero-count contract. The reconcile core already emits empty artifacts for a zero-finding review on the CLI path too, so the behavior is internally consistent.
**Fix in:** v2 — pre-check the sources tree in handleReconcile (mirror resolveReviewDir) before RunReconcile so both layers fail-before-emit; have gitrange.Resolve return resolved refs alongside ErrEmptyRange so handleRange can echo them.
