...
**Why accepted:** The acceptance criteria pin plain SHA-256 with an exact empty-string digest constant (AC 03-01 Edge Case 1 = `e3b0c442...`, AC 03-04 pins plain-SHA-256 outputs). Switching to a keyed HMAC-SHA256 with a pepper — the correct hardening — changes every pinned digest AND requires a provisioned application secret that does not exist yet (secret provisioning is out of Phase 2 scope). The Phase-2 fix was limited to accurate docstring wording (pseudonymization + its bound). The threat also requires a real telemetry endpoint, which is empty/no-op this sprint (see TD-003).
**Fix in:** Whichever sprint provisions a telemetry secret (coordinate with the `atcr.dev` backend owner, same as TD-003/TD-006) — replace the bare digest with `hmac.New(sha256.New, pepper)`, update AC 03-01/03-04's pinned digests to the keyed values, and keep cross-process determinism. Requires an AC change, so it cannot land inside Phase 2's pinned spec.
