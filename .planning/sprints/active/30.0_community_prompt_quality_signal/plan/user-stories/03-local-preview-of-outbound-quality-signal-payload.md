...
- **Assumptions:**
  ...
  - `--preview` is registered on both `atcr review` and `atcr reconcile` (the two commands that trigger quality-signal aggregation/transmission, matching Story 6's two Send call sites), via an `addQualitySignalFlags`-style helper in `cmd/atcr/flags.go`, following the existing `addSyncCloudFlags` pattern.
