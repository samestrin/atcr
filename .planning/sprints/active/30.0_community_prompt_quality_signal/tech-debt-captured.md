...
**Issue:** The frontmatter `description:` enumerates the routable commands (review … version) but stops before `config` and now `quality-report`, so a maintainer scanning only the description does not see either command. The `## Dispatcher` routing table (canonical, drift-tested) and `skill/skill_test.go`'s `dispatcherCommands` list were both updated for `quality-report`; only the prose enumeration lags. `config` was already absent, so this is a pre-existing non-exhaustive-list gap this sprint did not widen structurally.
**Why accepted:** The documented drift-catching invariant (SKILL.md comment lines 84-87) covers only the routing-table row + `dispatcherCommands` list — both were updated — not the frontmatter prose. The frontmatter is a curated ~500-line-budgeted description where the routing table is authoritative; the omission is discoverability-only, no functional/routing impact.
**Fix in:** Post-sprint docs pass — append `quality-report` and `config` to the frontmatter description enumeration, or add a note that the routing table is the canonical command list.
