...
## TD-007 — scorecard package writes diagnostics to os.Stderr, not the command writer (LOW)
**Origin:** Phase 3, task 3.2.A adversarial review, 2026-06-15
...
**Issue:** Store/emit diagnostics ("skipping malformed record", "run spans adjacent month files", "write failed") are written directly to the process-global `os.Stderr` rather than to a writer threaded from the cobra command (`cmd.ErrOrStderr()`). For real CLI use this satisfies the AC's "warn to stderr" requirement, but any caller that redirects cobra's error stream (or a test using `execCmdCapture`) cannot capture these lines.
**Why accepted:** This is the established convention across the whole `internal/scorecard` package (and mirrors `ReadRecords`/`Emit` written in Phases 1-2). Changing it for only the new Phase 3 code would be inconsistent; doing it package-wide is a cross-cutting refactor (thread an `io.Writer` or `*slog.Logger` through `Append`/`ReadRecords`/`Emit`/`FindByRunID`) out of scope for this task. The diagnostics still reach the terminal in real use.
**Fix in:** A follow-up package-wide change — accept an `io.Writer` (or logger) on the store/emit entry points and have the CLI pass `cmd.ErrOrStderr()`, so warnings are capturable and redirectable.
