...
**Resolved:** 2026-06-17 — Implemented option (a): `internal/log/handler.go` adds a `redactingHandler` (scrubs record message + string/error/group attr values) and a non-breaking `WithRedactor(logger, *Redactor)` helper that wraps the existing handler. Wired at both review correlation points — CLI `cmd/atcr/review.go:runReview` (root `prep.Repo`) and MCP `internal/mcp/handlers.go:reviewContext` (root `e.root`) — so every review log line is redacted by construction at every level. 100% coverage on internal/log retained (AC8); raised to HIGH at the Phase 4 gate (4.5) and fixed before the phase boundary.
...
## TD-011 — 429/503 retry backoff ignores the provider Retry-After header (LOW)
**Origin:** Phase 4, task 4.3.A adversarial review, 2026-06-17
**File:** internal/llmclient/client.go:262-267
**Issue:** The retry loop uses a fixed exponential backoff (initialBackoff * factor^attempt) and never inspects a `Retry-After` header on a 429/503 response. A rate-limited provider that advertises a cooldown can be retried before that window elapses, deepening the rate-limit rather than respecting it. Attempt count is correctly bounded (attempt <= maxRetries), so this is pacing-only, not a retry-storm.
**Why accepted:** Pre-existing backoff behavior, not introduced by 4.3 (which only adds error classification). Honoring Retry-After is provider-health pacing, explicitly out of scope for Epic 4.0 — it belongs with Epic 4.5 (Circuit Breaker / Provider Health), which consumes ClassifiedError to decide when a provider is failing.
**Fix in:** Epic 4.5 — parse `Retry-After` (delta-seconds or HTTP-date) from 429/503 responses and override the backoff delay when present; fall back to fixed backoff otherwise. Add a covering test asserting the honored delay.

## TD-012 — fanout review.go stderr warnings not yet routed through the injected logger (LOW)
**Origin:** Phase 4, task 4.5 phase gate review, 2026-06-17
**File:** internal/fanout/review.go:296,310,314,321,604
**Issue:** Five `fmt.Fprintf(os.Stderr, ...)` warnings (tool-harness/snapshot disabled, transcript dir mkdir, fallback-agent config override) still write directly to stderr, bypassing the single-sink contract and the new sink-level redaction. The four in `ExecuteReview` (296/310/314/321) already have `log.FromContext(ctx)` in scope; `buildFallbackAgent` (604) has no logger in scope (called pre-correlation from the build path).
**Why accepted:** Out of Phase 4's scope — task 4.2 explicitly scoped the stderr migration to `logSkepticFailure` (the highest-frequency, per-skeptic-per-finding site, now migrated). Original-requirements lists "rewriting every legacy fmt.Fprintf site in one pass" as Out of Scope; migration is incremental per package. These are one-time harness/config warnings (low frequency, low leak surface), not skeptic-failure detail.
**Fix in:** Future logging-migration pass — replace the four ExecuteReview sites with `log.FromContext(ctx).Warn(...)` (they then ride the correlated, redacted sink); thread a build-time logger into `buildFallbackAgent`/`PrepareReview` for site 604, or accept it as a pre-correlation build warning.
