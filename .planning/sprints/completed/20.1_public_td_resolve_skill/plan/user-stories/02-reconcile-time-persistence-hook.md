...
3. Re-running `atcr reconcile` against the same or a different review directory accumulates findings in the store rather than overwriting or losing prior runs' records, using write-time dedup by `FindingID` with a full-history `ReadAll` scan before each append (see AC 02-03). A finding whose `problem` text changes across runs is treated as a distinct record, which is documented rather than treated as a defect.
...
|------|--------|------------|
| Re-running `atcr reconcile` on the same review directory (e.g., after a partial fix) silently duplicates records in the append-only store, inflating the backlog with stale/duplicate entries | Medium | Closed during design: write-time dedup by `FindingID` using a full-history `ReadAll` scan before each append. A dedup-read failure fails open (append anyway) and is logged. A finding whose `problem` text changes is treated as a distinct record and documented as such. |
