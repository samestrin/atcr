...
> **Decision:** month sharding is the chosen path because it matches the two existing append-only ledgers (`scorecard`, `history`) and avoids the merge-conflict churn that motivated Epic 12.1's sharded-YAML migration. Per-run shards or a single `.atcr/debt.jsonl` were considered and rejected during design; the persistence hook derives the shard name from each record's `run_id` prefix.
...
- **Severity escalation is not surfaced (first-seen severity is frozen).** Because write-time dedup keys on `id` alone, a later run that re-discovers the same `file`/`line`/`problem` at a *higher* severity is skipped as a duplicate — the store keeps the severity from the first time the finding was persisted. `atcr debt resolve` selection then sorts and filters strictly on that frozen persisted severity with no live re-check. Surfacing escalation (comparing severity on a matched id and appending an updated record when it rises) is a real, worth-noting consequence and is intentionally deferred to a future epic (TD-001).
...
- **Dedup strategy:** write-time dedup by `id`. Before calling `Append`, the persistence hook reads all existing records (`ReadAll`) and skips any finding whose `id` already exists. This scope is full-history (all shards), not per-month, so a finding does not silently re-duplicate across a month boundary. If the dedup read fails, the hook fails open toward append (at-least-once semantics) rather than dropping the run's findings. This decision is recorded in AC 02-03.
