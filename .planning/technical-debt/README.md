...
**Last Modified:** 2026-07-06 | **Open Items:** 3 | **Deferred Items:** 49 | **Resolved Items:** 5 | **Total Items:** 57
...
|-------|---|----------|------|---------|-----|----------|-------------|--------|---------|----------|
| 1 | [ ] | LOW | internal/fanout/artifacts.go:214 | The TD-019 memo short-circuit only fires for parsedFindingCount==0, so a truncated-with-findings result (memo set to >=1 by the failover gate) re-runs stream.ParseModelOutput at line 217 — the redundant parse the refactor aimed to remove, though bounded to the rare truncated-with-findings path. | Cache the parsed []Finding slice on Result rather than only its length so findingsFor can reuse it, or accept the bounded cost. | performance | 15 | code-review | claude | MEDIUM |
...
| 1 | [ ] | MEDIUM | internal/fanout/engine.go:570 | The failover demotion gate uses the RAW ParsedFindingCount() (stream.ParseModelOutput), but the persisted FindingsCount and the run-level truncated_zero_findings tally (artifacts.go:134) use the GROUNDED count after groundFindings + enforceConstraints. A truncated response that raw-parses >=1 finding but has them all dropped as ungrounded/below-min-severity stays StatusOK (guard does not fire) yet is tallied as a runaway; the comment at artifacts.go:38-43 claims the tally is the rate the guard reacts to, which is false. Same divergence already deferred as the epic independent-review MEDIUM; corroborated by both reviewers. | Gate the failover on the same effective/grounded finding set findingsFor uses (thread grounding into invokeSlot), or correct the artifacts.go:38-43 comment to say the tally measures truncated agents with zero GROUNDED findings at final-status granularity. Needs an engine API change; deferred. | correctness | 60 | code-review | claude | MEDIUM |
