...
### [2026-06-21] From Sprint: epic-6.2

| Group | | Severity | File | Problem | Fix | Category | Est Minutes | Source |
|-------|---|----------|------|---------|-----|----------|-------------|--------|
| 1 | [ ] | LOW | internal/debate/cluster.go:74 | Two co-located distinct clusters sharing the same longest-member problem text collide in indexClusters (map overwrite) so an item may resolve to the wrong cluster ID, leaving residual over-suppression for the shared-longest-problem sub-case (pre-existing in indexClusters; also affects ruling application at debate.go:208). | Key indexClusters on the cluster's stable ID rather than the display-problem triple, or detect/log a clusterDisplayProblem collision so the residual co-location case is observable. | EDGE_CASES | 30 | execute-epic-independent |
| 1 | [ ] | LOW | internal/debate/cluster.go:74 | A gray item whose representative problem drifted (absent from clusterIdx) silently re-debates even when its cluster was in fact merged, re-billing a real merged cluster through a full debate (a re-run cost regression vs the old location-only filter for the drift case). | Add a location-keyed fallback restricted to merged ClusterIDs whose survivor sits at the item File+Line, or emit a debug log when a gray item misses clusterIdx so the re-debate cost is observable. | ERROR_HANDLING | 30 | execute-epic-independent |
| 1 | [ ] | LOW | internal/debate/cluster.go:64 | filterMergedClusters logs nothing about how many gray-zone items were suppressed by cluster identity, so an idempotency mis-fire (wrong ID match or unexpected pass-through) surfaces nothing in prod. | Add a debug-level log of the suppressed-item count and any ClusterMerged record carrying an empty ClusterID so legacy/self-heal paths are visible. | CORRECTNESS | 15 | execute-epic-independent |
| U | [ ] | LOW | internal/reconcile/emit.go:116 | External consumers that strictly validate findings.json (DisallowUnknownFields-style parsers) will reject merged records carrying the new cluster_id key, and no schema-version bump accompanies the additive field (matches the pre-existing cluster_merged convention). | Confirm no downstream parser uses strict unknown-field rejection on findings.json and document the additive cluster_id field in the findings.json schema reference. | CORRECTNESS | 15 | execute-epic-independent |
