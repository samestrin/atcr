...
**Last Modified:** 2026-06-23 | **Open Items:** 0 | **Deferred Items:** 46 | **Resolved Items:** 24 | **Total Items:** 70
...
| 1 | [x] | MEDIUM | cmd/atcr/github.go:220 | Fallback per-comment loop has no context-cancellation check between iterations | Add a fallback test where GET /comments returns one pre-existing "ATCR found:" comment matching one finding; assert returned deduped==1 and only the non-deduped comment is POSTed individually. | correctness | 30 | code-review | bruce | MEDIUM |
...
| 1 | [x] | MEDIUM | cmd/atcr/github.go:220 | Fallback path posts N comments sequentially without delay, could hit GitHub API rate limits for PRs with many findings | Add a fallback test where GET /comments returns one pre-existing "ATCR found:" comment matching one finding; assert returned deduped==1 and only the non-deduped comment is POSTed individually. | correctness | 30 | code-review | bruce | MEDIUM |
| 1 | [x] | MEDIUM | cmd/atcr/github.go:220 | Fallback path posts N comments sequentially without delay, could hit GitHub API rate limits | Add a fallback test where GET /comments returns one pre-existing "ATCR found:" comment matching one finding; assert returned deduped==1 and only the non-deduped comment is POSTed individually. | correctness | 30 | code-review | bruce | MEDIUM |
| 1 | [x] | MEDIUM | cmd/atcr/github.go:220 | Fallback path posts N comments sequentially without delay, risking GitHub secondary rate limits | Add a fallback test where GET /comments returns one pre-existing "ATCR found:" comment matching one finding; assert returned deduped==1 and only the non-deduped comment is POSTed individually. | correctness | 30 | code-review | bruce | MEDIUM |
