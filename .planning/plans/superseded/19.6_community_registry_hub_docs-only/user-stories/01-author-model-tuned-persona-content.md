...
- **Extended Scope Note:** The original request illustrated the gap with 1–2 model-tuned personas (Claude, GPT-4). This story implements the refined plan's 3-provider scope (Anthropic Claude, OpenAI GPT, Google Gemini) to give new users a frontier-model-diverse default panel; the additional Google Gemini persona is additive scope approved in the refined plan and does not replace any existing personas or the user's legacy-ported panel.
