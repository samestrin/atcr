...
- **Terminal poll states (Epic 1.5):** the poll loop stops when `atcr status` reports any terminal state — `completed`, `failed`, **or `stale`**. `stale` is the inferred dead-review state (`summary.json` absent and `started_at + timeout_secs + 60s` grace elapsed); the loop MUST treat it as terminal and stop, not keep polling a fan-out that will never complete. Distinguishing `stale` from `in_progress` is what lets the Skill tell a running review from a dead one (it no longer waits the full retry budget on an orphan, and should not keep `.atcr/latest` trusted as a live run). See the AC 04-04 `stale` contract amendment.
