...
The public submission JSON preserves model identifiers, persona names, role, and all numeric metrics exactly, and always declares `schema_version: 1`.
...
- **Then** each record contains all 16 required fields: `index`, `reviewer`, `model`, `role`, `runs`, `findings_raised`, `findings_corroborated`, `findings_solo`, `corroboration_rate`, `findings_verified`, `findings_refuted`, `survived_skeptic_rate`, `cost_usd`, `tokens_in`, `tokens_out`, `latency_ms_avg`
