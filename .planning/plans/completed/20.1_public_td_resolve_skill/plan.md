...
- New `internal/localdebt` package copies `internal/scorecard/store.go`'s Append/ReadRecords/ReadAll pattern (one `os.Write` per record under `O_APPEND` for atomic-append safety) but roots at `.atcr/debt/` instead of `os.UserConfigDir()`.
