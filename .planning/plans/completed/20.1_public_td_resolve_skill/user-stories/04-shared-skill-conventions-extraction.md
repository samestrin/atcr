...
- **Constraints:**
  ...
  - `TestSkill_NoAbsoluteOrClaudePaths` iterates a fixed list of embedded-MD variables (skill/skill_test.go:114) — `ConventionsMD` must be added to that list so the new file is held to the same no-`.claude`/no-absolute-path bar as the existing three.
