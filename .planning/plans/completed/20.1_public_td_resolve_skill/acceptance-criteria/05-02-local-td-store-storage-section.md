...
- **Given** Story 2 documents write-time dedup by `FindingID` using a full-history `ReadAll` scan before each append
- **When** the subsection describes cross-run accumulation
- **Then** it states that behavior explicitly (not left ambiguous), so a user re-running `atcr reconcile` on the same repo understands that unchanged findings will not duplicate, but a finding whose `problem` text changes is treated as a distinct record
