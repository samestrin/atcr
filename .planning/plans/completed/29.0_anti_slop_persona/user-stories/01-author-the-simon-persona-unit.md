...
- **Background:** ATCR's community registry (`personas/community/`) ships human-named persona units, each a `<name>.yaml` binding (provider/model/persona/role) paired with a `<name>.md` Go-template prompt. `sonny.yaml`/`sonny.md` is the closest structural analog: an openrouter-bound reviewer persona with the canonical `## Role` / `## Scope` / `{{if .ToolsEnabled}}...{{end}}` / `## Severity Rubric` / `## Output Format` / `## Payload` section order and a single leading `<!-- vendor-guidance: ... -->` citation comment. This story produces only the persona unit itself — `simon.yaml` and `simon.md` — not the fixture test or the `index.json`/registry wiring, which land in later stories of this plan. That auto-discovery cuts both ways at landing time: `CommunityNames()` lists the embedded directory directly (`personas/community.go:38`), so the moment `simon.md` is committed without Story 2's roster row, `TestCommunityAccessors`' `require.Len` (`personas/community_test.go:177`) fails fatally (14 names vs 13 rows) and `TestTemplateFixtureRunner_CommunityPersonasPass` (`internal/personas/community_fixture_test.go:17`) reddens until `simon_fixture.patch` exists. Stories 1 and 2 are therefore developed in story order but merge to the default branch as a single green unit — this story's files never land independently.
...
| Story 1's files are merged without Story 2's roster row and fixture, leaving `TestCommunityAccessors` (`require.Len` 14-vs-13, fatal) and the embedded fixture gate red on the default branch | High | Keep Stories 1 and 2 on one branch and merge them together as a single green change; treat this story's "done" state as "ready for Story 2's registration rows," and run `go test ./personas/... ./internal/personas/... ./internal/registry/...` only after both land. |
