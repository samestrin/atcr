...
| 2 | Adversarial/Security Pass | Fix | M | 6 sweep categories executed with reproducible commands; findings logged in `atcr-findings/v1` format at `code-review/adversarial-findings.txt` | Manual verification (reproducible grep/lint commands, documented in task) |
