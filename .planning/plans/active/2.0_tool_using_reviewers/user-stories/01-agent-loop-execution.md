...
- **Implementation Notes:**
  ...
  - **PayloadContext** (internal/payload/template.go:15): Add `ToolsEnabled bool` field. Set by `buildAgent` (review.go:411) from `AgentConfig.Tools` before calling `RenderPrompt`. Enables `{{if .ToolsEnabled}}` persona sections in downstream story.
