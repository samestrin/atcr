...
- `internal/doctor/render.go` - reference: `tabwriter` usage pattern (`tabwriter.NewWriter(w, 0, 2, 2, ' ', 0)`)
...
4. `TestRenderTable_SingleReviewer` — single-row table renders correctly
...
7. `TestRenderTable_LongNames` — columns expand correctly
