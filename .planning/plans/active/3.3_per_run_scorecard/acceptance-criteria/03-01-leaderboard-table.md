...
- `internal/doctor/render.go` - reference: `tabwriter` initialization and column alignment pattern
...
**Scenario 3: Corroboration rate computed correctly across runs**
...
**Scenario 4: Cost per corroborated finding computed correctly**
- **Given** `diana`/`gpt-4o` has total cost $0.1500 and 5 corroborated findings across all runs
- **When** `atcr leaderboard` is executed
- **Then** the row shows: total cost=$0.1500, cost per corroborated finding=$0.0300
