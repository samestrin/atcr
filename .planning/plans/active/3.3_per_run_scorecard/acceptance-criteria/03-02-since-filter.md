...
| Timestamp Source | `run_id` field (ISO timestamp prefix) or `timestamp` field on record | Compare against `time.Now().Add(-duration)` |
...
- `internal/scorecard/store.go` - reference: record struct with `RunID` and/or `Timestamp` fields
