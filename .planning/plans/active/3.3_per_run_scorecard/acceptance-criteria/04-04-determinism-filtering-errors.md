...
- **Given** a `.jsonl` file in the scorecard store contains a malformed JSON line
- **When** `atcr leaderboard --export` is executed
- **Then** the command exits with code 1 and prints an error identifying the corrupt file and line number; no partial export is produced
