...
- `personas/community/index.json:1` — modify: for any entries that gain an authored `binding` value, the existing `model` value is left byte-for-byte unchanged — proving "zero migration": the pre-story `model` value already IS the initial lock, requiring no rewrite, backfill command, or data transform to become one.
- `internal/personas/search_test.go:30` (`verifyCommunityIndex`) — reference/modify: the function's exact-match loop checks only `Provider`/`Model` equality against the source YAML (plus the path-escape/read checks); it does not enumerate `Binding` at all. Add a small regression test proving this explicitly (see Edge Case 1) so the exemption is asserted, not merely assumed by omission.
- `personas/community_test.go:330` (`TestCommunityIndex_Registration`) — modify: this test additionally checks `Description` equality (`:356-367`); extend it (or add a sibling assertion) so that when an entry carries a `binding` value it is compared for equality against the source YAML's `binding:` key — keeping the new field in sync via the same test that already guards `Provider`/`Model`/`Description`, without weakening any of those three existing checks.
...
- `internal/personas/unit.go:95` (`writePersonaUnit`) — reference only: the shared paired-write tail used by both `InstallUnit` and `Upgrade`; the lock (the existing `model` value) is persisted through here with zero migration.
- `docs/personas-authoring.md` — modify: document that a persona's existing `model` value seeds the initial resolved lock with zero migration.
