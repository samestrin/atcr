...
- **Background:** `cmd/atcr/init.go:96` (`installCommunityPersonas`) is the single shared install routine called from both `init.go:47` and `quickstart.go:102`. Both call sites loop `builtins.Names()` — the 9 embedded, model-agnostic built-in personas (`bruce, greta, kai, mira, dax, sasha, penny, ingrid, otto`, from `personas/personas.go:19`) — against the fetched community index. The shipped `personas/community/index.json` publishes a completely disjoint set: 10 model-indexed library personas (`anthony, sonny, gene, milo, gia, flint, delia, quinn, ...`), each bound to a specific provider/model pair. Because the two name sets share no members, every online `init`/`quickstart` run today pins zero community personas and prints all nine of `installCommunityPersonas`'s `%q not found in community index — skipping` warnings (`init.go:129`) — noisy output for a command that silently does nothing. This is TD-011, deferred out of 19.6 as a HIGH because closing it correctly requires deciding what the "community roster" for init/quickstart should actually be, which this story now resolves. The 19.6 postmortem (documented in `documentation/existing-resolver-patterns.md`) also flags that `init.go` and `quickstart.go` have drifted from each other before (TD-006/TD-007) when a fix landed in one call site but not the other — this story must fix the shared `installCommunityPersonas` routine (or its shared caller-side roster input) exactly once, not patch each call site independently.
- **Assumptions:** The exact reconciliation strategy is intentionally undecided at the plan level — the Proposed Solution (item #8) offers two options and defers the choice to design-sprint/implementation judgment: (a) publish the built-in lenses into the community channel behind a model-agnostic gate carve-out, preserving the constraint that built-ins stay model-agnostic (C2), or (b) align the fetch-and-pin roster passed to `installCommunityPersonas` with whatever the index actually publishes (the 10 model-indexed personas), dropping the now-misleading skip-warning path for names that were never meant to be in the index. Either resolves the disjoint-roster bug; this story does not pre-select one. Existing on-disk community personas already installed by prior runs must remain untouched (`installCommunityPersonas`'s existing "already installed — leaving it untouched" guard at `init.go:139` must continue to apply unchanged).
...
- **Achievable:** Both options in the Proposed Solution reuse the existing `installCommunityPersonas` machinery (fetch, rollback, never-overwrite guard) unchanged — the fix is scoped to what roster/index gets passed in, not to the install mechanics themselves.
...
- **Implementation Notes:** The fix is scoped to what roster/index `installCommunityPersonas` (`cmd/atcr/init.go:96`) is asked to reconcile against, not to its fetch/rollback/never-overwrite mechanics, which stay unchanged. Two neutral paths, both consistent with the Proposed Solution's item #8, are available and the choice is left to implementation/design-sprint judgment:
  - **Option A — publish built-ins into the community channel:** Add the 9 built-in names to `personas/community/index.json` (or an equivalent model-agnostic carve-out) so `builtins.Names()` resolves against the index as-is. Requires defining what "model-agnostic gate" means for an index whose other entries are all model-bound, and keeping the built-ins genuinely model-agnostic per C2.
  - **Option B — align the roster with the index:** Change the roster passed to `installCommunityPersonas` at both call sites (`init.go:47`, `quickstart.go:102`) from `builtins.Names()` to the set the index actually publishes, and drop (or repurpose) the now-unreachable skip-warning path for names never meant to be in the index.
  Whichever path is chosen, it must be expressed once — as a shared roster/index source both call sites consume — not duplicated per call site, to avoid repeating the TD-006/TD-007 drift pattern.
- **Integration Points:**
...
- **Data Requirements:** No new persisted state or lock format; this story only changes what set of names is reconciled against the index (and, if Option A is chosen, the contents of `personas/community/index.json` itself). Existing on-disk `.atcr/personas/` community-tier files and their pins are unaffected.

## Potential Risks

| Risk | Impact | Mitigation |
|------|--------|------------|
| Option A (publishing built-ins into the index) accidentally binds a built-in to a specific model, violating the model-agnostic constraint (C2) that built-ins must satisfy. | High | Define an explicit model-agnostic carve-out in the index schema/loader before adding built-in entries; add a test asserting built-in-sourced index entries carry no provider/model binding. |
| Option B (aligning the roster with the index) silently changes what persona set a fresh `init`/`quickstart` installs, surprising users who expected the 9 familiar built-in names as their community tier. | Medium | Document the roster change in the command's help text/README and in the CHANGELOG entry for this sprint; cover the new expected roster with an explicit test asserting the installed name set. |
