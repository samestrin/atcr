...
- **Background:** Story 1's catalog spike determines whether OpenRouter's `~`-prefixed `-latest` aliases are actually completion-routable, and Story 2 defines the family/channel binding data model and lock field that this resolver populates. This story is the resolution engine itself — the logic invoked exclusively from `Upgrade()` (`internal/personas/upgrade.go:27`) that takes a persona's declared family/channel and produces the concrete slug written into the lock. Of the 10 personas, 7 have a provider-owned `-latest` alias (Anthropic ×2, OpenAI ×2, Google ×2, Moonshot ×1) where the provider itself resolves to the newest member — near-zero resolver maintenance. The remaining 3 (delia→DeepSeek, quinn→Qwen, glenna→GLM) have no `-latest` alias in the catalog and require this story's `created`-timestamp newest-in-vendor-prefix strategy, or fall back to an explicit-slug pin as an always-available escape hatch.
- **Persona-to-Resolver Mapping:** The exact mapping, sourced from `personas/community/index.json`, is:
  - **Alias passthrough:** anthony (`~anthropic/claude-opus-latest`), sonny (`~anthropic/claude-sonnet-latest`), gene (`~openai/gpt-latest`), milo (`~openai/gpt-mini-latest`), gia (`~google/gemini-pro-latest`), flint (`~google/gemini-flash-latest`), celeste (`~moonshotai/kimi-latest`).
  - **`created`-timestamp scan:** delia (`deepseek/`), quinn (`qwen/`), glenna (`z-ai/` — **not** `glm/`).
  - **Explicit pin:** any persona whose binding is a concrete slug, regardless of vendor.
...
1. The 7 alias-covered personas (Anthropic ×2, OpenAI ×2, Google ×2, Moonshot ×1) resolve by returning the provider's `~`-prefixed `-latest` alias slug unchanged — the resolver performs no catalog scan for these, the provider owns resolution.
...
- [OpenRouter Catalog & Completions API](../documentation/openrouter-catalog-api.md) — model schema, alias behavior, and deprecation signal.
- [Existing Codebase Patterns to Reuse](../documentation/existing-resolver-patterns.md) — `fetch()` template, `Upgrade()` insertion point, and the `ResolvePersona` boundary rule.
- [Catalog Snapshot Fixture Discipline](../documentation/catalog-snapshot-fixture.md) — zero-live-network testing and the refresh command that keeps the fixture current.
