...
- **Integration Points:** The index-generation/publish tooling that currently emits `name`/`version`/`description`/`path` per persona (wherever `index.json` is built for the community repo) needs to also emit `provider`/`model`/`tasks`/`tags` sourced from each persona's parsed YAML. This story does not touch `Search()`'s matching logic — that consumes these new fields in a follow-on story. See `documentation/fetch-and-distribution.md` "Index.json Generation Workflow" for the recommended generation-script + CI-validation approach so the index never drifts from the YAML sources.
