...
**Open design questions the epic still needs to resolve.** Three integration gaps are flagged and unresolved in the discovery snapshot: (1) whether reconcile-time persistence appends every finding, only ones at/above a severity threshold, or dedups by `FindingID`; (2) whether `/atcr debt resolve` reads the local store's raw files directly (coupling the skill to the file format) or goes through a new `atcr debt resolve` CLI command (keeping SKILL.md's CLI-invocation-only rule); (3) whether `atcr reconcile` needs idempotency against already-persisted findings on re-reconcile of the same review directory, since the store is described as append-only.
> Source: [codebase-discovery.json: integration_gaps "Reconcile-time persistence granularity"]
> Source: [codebase-discovery.json: integration_gaps "Skill route store contract"]
> Source: [codebase-discovery.json: integration_gaps "Opt-out and idempotency"]
