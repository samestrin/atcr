...
Repo-root detection for the `.atcr/` path is not yet settled: `cmd/atcr/reconcile.go` uses CWD as repo root (`Root: "."`), and the new package should either follow that same convention or use a shared helper (e.g. `git rev-parse --show-toplevel`) so the store is always scoped to the current repo.

> Source: [codebase-discovery.json: integration_gaps: "Local store path resolution"]

Shard/deduplication strategy is also an open question: whether the local store shards by month (like scorecard/history), by `run_id`, or keeps a single `.atcr/debt.jsonl`, informed by the Epic 12.1 lesson on single files vs. per-run shards. A decision and a `schema_version` field are needed.

> Source: [codebase-discovery.json: integration_gaps: "Shard/deduplication strategy"]
...
| `internal/localdebt` (new, Epic 20.1) | `.atcr/debt/` (proposed; final path TBD) | Per-repo, standalone/public (zero `.planning/` dependency) | To be designed/implemented | TBD — month vs. run_id vs. single file (open integration gap) | Must state its own concurrency guarantee explicitly and reference TD-004 won't-fix |
