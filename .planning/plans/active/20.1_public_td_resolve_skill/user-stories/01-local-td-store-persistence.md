...
- **Implementation Notes:**
  ...
  - Path resolution: root at `<repo-root>/.atcr/debt/`, month-sharded as `YYYY-MM.jsonl` derived from the record's `run_id` prefix, mirroring `internal/scorecard/paths.go`'s `monthFromRunID`. Repo-root detection should follow `cmd/atcr/reconcile.go`'s existing `Root: "."` (CWD) convention rather than inventing a new resolution strategy, per the open integration gap noted in `documentation/append-only-store-pattern.md`.
...
| Dedup strategy left undecided causes duplicate or conflicting records once Story 2 starts writing on every reconcile run | Medium | Resolve and document the dedup strategy (append-always + reader-side dedup by `id` vs. read-before-write skip) as part of this story's deliverable, not deferred to Story 2 |
