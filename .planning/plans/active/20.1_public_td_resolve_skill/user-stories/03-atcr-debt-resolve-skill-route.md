...
- **Constraints:**
  ...
  - Every command invocation must remain CLI-only (never a direct engine call), consistent with `skill/SKILL.md`'s stated dispatcher contract (`skill/SKILL.md:16,37`). Whether the skill reads `.atcr/debt/*.jsonl` directly or via a new `atcr debt resolve` CLI subcommand is an open design question this story must resolve (see Risks) — the CLI-subcommand path is preferred for consistency with the existing dispatcher pattern but requires new work in `cmd/atcr/debt.go`.
...
- **Integration Points:**
  ...
  - Local TD store (Story 1) and its reconcile-time persistence (Story 2) — the read/query surface this route consumes; open question whether access is direct file read of `.atcr/debt/*.jsonl` or via a new `atcr debt resolve` CLI subcommand in `cmd/atcr/debt.go` (design decision to resolve before/during this story).
...
|------|--------|------------|
| Direct-file-read vs. new-CLI-subcommand design question left unresolved could stall implementation or produce an inconsistent dispatcher pattern | Medium | Resolve explicitly during `/design-sprint`; default to a new `atcr debt resolve` CLI subcommand in `cmd/atcr/debt.go` for consistency with SKILL.md's CLI-invocation-only convention, per the discovery snapshot's stated preference |
