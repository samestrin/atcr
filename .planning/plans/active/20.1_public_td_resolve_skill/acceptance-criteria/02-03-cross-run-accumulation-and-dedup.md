...
Per the plan's explicit design instruction and `documentation/local-td-store-schema.md`'s open integration gap, this story resolves the dedup question as **write-time dedup**: before calling `localdebt.Append` for a given finding, the hook checks whether a record with the same `id` (`history.FindingID(file, line, problem)`) already exists in the current month's shard; if so, the finding is skipped (not re-appended). This is the safer default given the store is append-only with no read-time compaction yet (per the story's risk table). Read-before-write is O(shard size) per finding but is accepted at the documented scale (append-only ledgers, hundreds of records/month) — no index or database is introduced.
