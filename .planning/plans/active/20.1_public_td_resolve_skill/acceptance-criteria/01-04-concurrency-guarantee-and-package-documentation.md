...
**Edge Case 2: Dedup strategy decision is documented, not left open**
- **Given** the schema doc's noted "open integration gap" on dedup strategy (append-always + reader-side dedup by `id`, vs. read-before-write skip)
- **When** the `internal/localdebt` package doc comment (or `record.go`/`store.go` comment) is read
- **Then** it states which strategy this story's implementation adopts and why, so Story 2 (the `atcr reconcile` persistence hook) has a settled contract to write against rather than an unresolved question
...
- [ ] The dedup strategy (append-always + reader-side dedup by `id`, vs. read-before-write skip) is explicitly decided and documented, not left as an open question
