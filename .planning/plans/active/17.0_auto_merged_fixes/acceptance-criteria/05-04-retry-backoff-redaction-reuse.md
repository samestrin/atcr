...
- `internal/autofix/*.go` (Story 4's orchestration package) - modify: before calling `CreatePullRequest`/`UpdatePullRequest`, run any dynamic content sourced from validation stdout/stderr excerpts through `Client.redactSecrets` (or equivalent) so a PR body built from validation diagnostics cannot echo a token or credential captured during the local validation step (Story 2)
