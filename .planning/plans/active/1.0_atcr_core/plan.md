...
- **LLM Client Contract:** OpenAI-compatible POST `${base_url}/chat/completions` with `Authorization: Bearer ${api_key_env}`. Retry policy: 429/500/502/503/504 only, ~500ms initial delay, 1.5× backoff. Other 4xx fail immediately. API keys resolved from environment variables at invoke time, not load time. Per-agent `temperature` and `timeout_secs`. The retry budget must not exhaust the per-agent or global timeout.
