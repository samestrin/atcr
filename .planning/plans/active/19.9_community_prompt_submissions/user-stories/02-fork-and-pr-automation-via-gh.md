...
- **Constraints:**
  ...
  - The persona unit (`.yaml` plus optional co-located `.md`) must be copied from its resolved source — built-in embedded, installed on-disk under `commpersonas.PersonasDir()`, or embedded community library — into the fork's working tree before the branch is pushed.
...
- **Implementation Notes:** Add a `newPersonasSubmitCmd()` to `cmd/atcr/personas.go` following the existing `newPersonas<Verb>Cmd()` pattern. Introduce a package-level seam (e.g., a `personasGitHub` interface or package var) exposing fork/push/pr-create operations, with a default implementation backed by `gh.ExecContext(ctx, "repo", "fork", ...)`, git push, and `gh.ExecContext(ctx, "pr", "create", ...)`. Implement the precondition check (`gh.Path()` + `gh auth status` via `gh.ExecContext`) as a standalone function callable independent of the rest of the flow, per the code example already captured in `documentation/gh-fork-pr-integration.md`. Add `github.com/cli/go-gh/v2` to `go.mod` (`go get github.com/cli/go-gh/v2`) — it is not currently a dependency. Before shelling out, resolve the persona unit to copy: built-in personas ship embedded in the binary, installed personas live under `commpersonas.PersonasDir()`, and community-library personas ship embedded; copy the relevant `.yaml` (and `.md` when present) into the fork under the appropriate path so the PR contains the complete submission.
- **Integration Points:** `cmd/atcr/personas.go` (new `newPersonasSubmitCmd`, alongside existing `personasDir`/`personasClient`/`personasFixtureRunner` seams); `github.com/cli/go-gh/v2` top-level `gh` package and `pkg/repository` for target validation (see the package spec at [../../../specifications/packages/go-gh.md](../../../specifications/packages/go-gh.md)); the Theme 1 fixture-gate call (`commpersonas.TestPersona`) as the immediate predecessor step whose success gates entry into this flow. Explicitly does not integrate with `internal/ghaction.Client` (internal/ghaction/client.go:192, :345) — that remains scoped to the Epic 17.0 `--auto-fix` bot flow.
