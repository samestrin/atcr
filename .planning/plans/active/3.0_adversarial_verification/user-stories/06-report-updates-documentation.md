...
- **Reference:** [Adversarial Verification Interface](../../specifications/design-concepts/adversarial-verification-interface.md) — defines the report rendering conventions for the Skeptic section, collapsed Refuted section, and v2 confidence badge ordering.
- **Assumptions:**
...
6. `docs/findings-format.md` is updated to document the per-finding `verification` block (verdict, skeptic, notes) and the v2 confidence tier (`VERIFIED`).
...
- **Implementation Notes:**
  ...
  - **`docs/findings-format.md` update:** Document the `verification` block inside `JSONFinding`: fields (`verdict`, `skeptic`, `notes`), allowed verdict enum (`confirmed|refuted|unverifiable`), and the new v2 confidence tier (`VERIFIED`). Clarify that the block is omitted in 1.x output and populated only after the verify stage.
