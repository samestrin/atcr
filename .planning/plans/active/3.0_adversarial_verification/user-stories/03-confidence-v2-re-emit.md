...
- **Assumptions:**
  - Stories 1 and 2 are complete and provide `SelectEligibleSkeptics` and `invokeSkeptic` (or a pipeline function that returns `[]*reconcile.Verification` per finding).
...
- **Implementation Notes:**
  ...
  - **Gate counter update (`internal/reconcile/gate.go`):** The `CountAtOrAbove` function (line 57) currently counts findings with confidence at or above a threshold. This story modifies it to exclude findings with `verdict="refuted"` (or equivalently, confidence="LOW" and `Verification.Verdict="refuted"`). The function signature does not change; the filtering logic is added internally. Alternatively, a new helper `failingFindingsFiltered(findings []reconcile.Merged) []reconcile.Merged` can be added that filters out refuted findings, and `CountAtOrAbove` calls this helper before counting. The choice depends on how the gate is currently implemented; the goal is to ensure `--fail-on` excludes refuted findings.
  ...
  - **Integration with Story 2:** Story 2 produces `*reconcile.Verification` values per finding. This story consumes them. The interface between Story 2 and Story 3 is a `map[FindingKey]*reconcile.Verification` (or equivalent) passed to `ReEmitFindings`. The exact shape depends on how Story 2 structures its output; this story defines `FindingKey` and expects Story 2 to populate the map.
- **Integration Points:**
  ...
  - `internal/reconcile/gate.go` — `CountAtOrAbove` (line 57): gate counter modified to exclude refuted findings.
