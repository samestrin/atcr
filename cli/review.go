...
func newReviewCmd() *cobra.Command {
	...
	cmd.Flags().Int("max-tokens", 0, "output-token cap per reviewer call (overrides each agent's max_tokens and the built-in default). Raise it for reasoning/thinking models, which spend output budget on chain-of-thought and otherwise finish mid-reasoning and return an empty review. Note the cap is also reserved out of the context window when sizing the payload, so raising it narrows each agent's input budget. Like every CLI-tier override (--timeout, --max-parallel, --byte-budget, --payload), it is not recorded in the manifest: retype it on --resume to apply it to the pending agents, or they are sized from config alone.")
