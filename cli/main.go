// Package cli owns the atcr command tree and its process lifecycle: it builds
// the cobra command tree (NewRootCmd/NewRootCmdWithClient), wires signal
// handling, telemetry draining, and exit-code mapping, and exposes a single
// end-to-end entry point (Main) that cmd/atcr's thin shim calls.
//
// This package is deliberately TOP-LEVEL (github.com/samestrin/atcr/cli), NOT
// under internal/. The private atcr-enterprise repository is a SEPARATE Go
// module, and Go's internal-package rule forbids a different module from
// importing anything under github.com/samestrin/atcr/internal/... — so an
// internal/cli seam could never be imported by the enterprise wrapper (verified:
// `go build` reports "use of internal package ... not allowed"). Keeping the
// seam here lets that second module import it and construct the same tree,
// injecting enterprise hooks without vendoring or copy-pasting cmd/atcr. Do not
// move this back under internal/ (Sprint 34.0 Task 03).
package cli
...
// Main runs the atcr CLI end to end: it installs SIGINT/SIGTERM handling,
// constructs the process telemetry client and root command, executes the
...
func runMain(ctx context.Context, stdout, stderr io.Writer) int {
	...
	if err != nil {
		code := exitCode(err)
		if code != 0 {
			_, _ = fmt.Fprintln(stderr, "atcr:", terminalErrorLine(err))
		}
		return code
	}
	return 0
...
// terminalErrorLine renders a returned error for the terminal print above.
//
// It exists because that print is the ONE error path that never passes through
// the redactor setupLogger installs: the logger is request-scoped (built per
// invocation in cobra's PersistentPreRunE) and is long gone by the time
// ExecuteContext returns its error here. Every structured log line is scrubbed
// and this line was not, so a secret-shaped token echoed by a subprocess — the
// docker daemon's raw stderr is the measured case (TD, internal/verify/exec.go)
// — reached stderr and CI logs verbatim.
//
	...
	root := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			...
			if cmd.Flags().Lookup("axi") != nil {
				...
				// AXI output is actually requested. --dry-run (or its --preview alias,
				// via dryRunFlagSet) is skipped: it short-circuits to the quality-signal
				// preview in runReview before any pipe renderer is selected, so the
				// notice would describe output that never happens (TD: cli/review.go:283).
				if axi && legacy && !dryRunFlagSet(cmd) {
					if legacyFlag {
