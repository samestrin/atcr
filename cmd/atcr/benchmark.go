...
func newBenchmarkRunCmd() *cobra.Command {
	...
	cmd.Flags().String("checkpoint", "", "opt-in: path to a run checkpoint file. Each scored case is durably recorded here before the next begins; re-running the same suite resumes from the first unscored case instead of restarting (and re-paying for) the whole run. The path must not be shared across concurrent benchmark run invocations. Empty = no checkpointing (default).")
	_ = cmd.MarkFlagRequired("suite-path")
	return cmd
}
