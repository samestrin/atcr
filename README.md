...
| `atcr diff-smell` | Scan a unified diff for over-simplification ("reward-hack") fingerprints — a deleted or skipped test, a weakened assertion, a lint suppression. Deterministic and model-free (no provider, no API key, no network) — see [docs/diff-smell.md](docs/diff-smell.md). Promoted from `atcr verify diff`, which stays a hidden deprecated alias for one minor version |
...
**Command-group nouns are singular** — `benchmark`, `config`, `debt`, and `skill` follow the git/docker/kubectl/gh convention of a singular noun for a group holding subcommands, and any new group keeps that rule. `models` and `personas` are **grandfathered** plurals: they shipped plural, and renaming them would break every scripted and documented invocation for a consistency win no user feels, so they stay. (`debt` is a mass noun and `config` names one file — neither could be pluralized anyway.)

Key flags:

- `atcr review --base X --head Y` / `--merge-commit SHA` / `--id <id>` / `--output-dir <path>` (write the tree to an explicit path; see below) / `--payload diff|blocks|files` / `--timeout <secs>` / `--fail-on <severity>` (one-shot review + reconcile + gate) / `--resume <latest\|id\|path>` (finish an interrupted/failed review by running only its pending agents, then reconcile; see below) / `--force` (overwrite an existing `--id` or `--output-dir` collision, backing the prior tree up to `<dir>.bak` first; mutually exclusive with `--resume`) / `--no-cache` (bypass the diff cache read and force a fresh review; fresh results are still written back to `.atcr/cache`) / `--sprint-plan <path>` (inject a sprint/epic plan as a `SCOPE CONSTRAINT` before the diff so reviewers suppress findings unrelated to the plan's work items; a missing/empty plan is ignored, an unreadable one warns and proceeds) / `--pr <n>` (stamp the pull-request number on this run's audit record; falls back to `GITHUB_REF` when unset) / `--all` / `--scope <path>` (baseline whole-repo or subtree review — see below; `--dir` is a deprecated alias) / `--no-file-cache` (bypass the baseline incremental cache and rescan everything; `--fresh` is a deprecated alias) / `--no-ignore` (include files matched by `.gitignore` / `.atcrignore`) / `--auto-fix` (apply sandboxed fixes and open a PR — see below) / `--axi` (token-dense TOON output for agent callers — see below) / `--sync-cloud` / `--cloud-endpoint <url>` (upload this run's telemetry; requires `ATCR_API_KEY`) / `--dry-run` (print the quality-signal payload without sending it; `--preview` is a deprecated alias) / `--byte-budget <n>` / `--max-parallel <n>`
...
- `atcr report --format md|json|checklist|sarif|axi|pipe` (`pipe` is the deprecated legacy AXI encoder) / `--output <file>` / `--disagreements` (focused disagreement-radar view — see [docs/disagreement-radar.md](docs/disagreement-radar.md))
...
`atcr review` does not require a diff range: `--all` scans the whole repository and `--scope <path>` scans a subtree — useful for a first pass over a legacy codebase or a periodic full audit. Baseline runs are incremental: files whose content hash is unchanged since the last baseline are skipped, and `--no-file-cache` forces a full rescan. `--all` / `--scope` are mutually exclusive with `--base` / `--head` / `--merge-commit`.
