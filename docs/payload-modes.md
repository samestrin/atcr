...
- **Identically for every agent in a fan-out.** The ledger is built once per review range and is exempt from every byte-budget shed, so no reviewer receives a different set of claims from any other.
