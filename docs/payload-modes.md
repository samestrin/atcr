...
> **Chunk sizing is a separate knob.** `payload_byte_budget` governs this shed — whole files, gone before any reviewer sees them. `chunk_byte_budget` governs how large one chunk may be under `review_strategy: chunked`. Unset, it inherits `payload_byte_budget`; set it when you need a large payload (shed nothing) delivered in small chunks (fit a narrow window). No `on_overflow` setting recovers a file this shed dropped — that policy applies only to the later per-agent step.
...
- **Identically for every agent in a fan-out, on the normal path.** The ledger is built once per review range and the byte-budget shed passes over it, so no reviewer receives a different set of claims from any other. Three exceptions are known and deliberate, recorded in the code that creates them (`internal/payload/claims.go`). Each leads with the condition that triggers it:

  - **Under `review_strategy: chunked`** — the ledger rides the **first chunk only**, because chunking splits the payload text on diff-file markers and the ledger sits above the first of them.

    The pre-fetched **Context Definitions** block sits in that same preamble and shares this fate exactly: it too rides the first chunk only. It carries one consequence the ledger does not. The [Epic 14.1 grounding gate](#grounding) widens groundability to the retrieved spans **review-wide**, and that widening is kept only while *every* dispatched agent can be shown to have received the block. On a **multi-chunk** run the agents reviewing chunks 2..N did not receive it, so the widening is **revoked for the whole review** and findings on merely-referenced files are dropped — the correct outcome rather than a defect, since those reviewers genuinely never saw the retrieved context. A chunked run that produces a **single** chunk is unaffected: the chunk-slot loop is guarded on `len(chunks) > 1`, so a one-chunk run falls through to the bulk path, its slot carries the block as an ordinary entry, and the widening survives. The revocation is never silent — it is recorded as `grounding_revoked` in the run's `status.json` and emitted as a `warn` line naming the agents that could not be shown to have kept the block.
  - **When an agent's declared window drives its effective budget to `0`** — that agent ships exactly one entry, which may be the ledger **alone**. The `NOT-IN-PAYLOAD` verdict above is what keeps this case from producing a sheet of false `UNSUPPORTED` findings.
  - **Under `on_overflow: truncate`, when a fallback's budget cannot fund both the ledger and a reviewable file** — the fallback re-fits without it, because the re-fit sizes every entry by the bytes it will actually dispatch, and that reviewer receives no claims section at all. Two routes shed it, so a budget larger than the ledger is not enough to keep it: the exemption's own `clampSize(Size) <= budget` bound drops a ledger larger than the budget, and a budget that clears that bound but cannot also fund a file sheds every reviewable file to fund the ledger — which trips `AllDropped` and reroutes to `keepSmallestEntry` (`internal/fanout/review.go:3565-3571`). That reroute keeps the smallest **entry**, so it drops the ledger only while some reviewable file is smaller than the ledger; when every file is larger, the ledger is what survives and the fallback receives claims and no code (see below).
...
**Two costs of the exemption, worth knowing:** on the ordinary shed the ledger's bytes are not counted against `payload_byte_budget` or against any per-agent window (hence the deliberately small 8 KiB cap) — the one exception is the fallback re-fit, which re-sizes every entry to the bytes it will actually dispatch and so counts the ledger like any other entry — and can therefore shed it whenever that budget cannot fund both the ledger and a reviewable file, either on the exemption's `clampSize(Size) <= budget` bound or through the `AllDropped` reroute to `keepSmallestEntry` (`internal/fanout/review.go:3565-3571`) that fires when funding the ledger would shed every reviewable file. That reroute keeps the smallest **entry**, so the ledger is what goes only while some reviewable file is smaller than it. Fitting the budget is therefore necessary but not sufficient — and not a guarantee of loss either. And it occupies one entry in the payload, so a range review's reported changed-file count is one higher than the number of files the range changed.
