...
2. **Copy or adapt the dispatcher template** from this repo's [`skills/atcr/SKILL.md`](../skills/atcr/SKILL.md) and all five of its on-demand siblings (`host-review.md`, `ambiguity-adjudication.md`, `findings-format.md`, `CONVENTIONS.md`, `debt-resolve.md`) — `CONVENTIONS.md` holds the shared prerequisites the dispatcher loads on demand, and `debt-resolve.md` backs the `atcr debt` route, so omitting either leaves references unresolvable. Alternatively, `atcr skill export --dest <path>` writes the complete tree (see [`docs/skill-usage.md`](skill-usage.md)).
