...
- **Resolution is currently terminal.** `atcr debt resolve --resolve <id>` appends a terminal `resolved` status record. Once an id has any terminal record, `selectOpenDebt` folds it out of the open backlog permanently, even if the same file/line/problem is re-detected by a later reconcile. A re-openable resolution mode (re-detected regressions re-enter the backlog) is deferred to a follow-up epic (TD-004).
