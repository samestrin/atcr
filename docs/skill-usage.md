...
- **Resolution lifetime depends on the status.** `atcr debt resolve <id>` appends a terminal status record. `wontfix` is permanent — suppression is the point of the flag. Every other status is **re-openable**: because the finding id includes the line number, re-detecting the same file, line, and problem text after a fix means a regression or a fix that never landed, so the id returns to the open backlog. That covers `resolved` and `deferred`, and also `unreproducible` (investigated, could not be reproduced — a re-detection is evidence it was real after all) and `attempts-exhausted` (the fix attempts ran out; the item stays in the live backlog because the work is unfinished). `--reason` is required for `wontfix`, `unreproducible` and `attempts-exhausted` — every status `atcr debt resolve` writes other than plain `resolved`. Deferral is not one of them: it is written by other paths, `debt resolve` rejects it as a `--status` value, and it carries no rationale. See [technical-debt.md](technical-debt.md) for the full lifetime table.
