...
- **A review payload is no longer only the diff.** Context pre-fetching retrieves short regions of files the diff never touched — any tracked file at the reviewed revision whose extension has a parser and which word-matches a changed symbol — so a file you did not edit can still reach a provider. It is ON by default, bounded by the `max_prefetch_bytes` ceiling, and only an explicit `max_prefetch_bytes: 0` disables it outright; see [registry.md](registry.md) for the default and the knob. The `.atcrignore` / `.gitignore` filter governs retrieved context exactly as it governs the diff, so paths you already exclude are never retrieved — that filter, an all-local roster, or setting the ceiling to `0` are the three controls here. There is deliberately no filename-based deny-list: matching names rather than contents would deny ordinary source (`settings.py`, `config.js`) while still missing a secret in a file named anything else, and it would drop context silently. **Retrieval reads more than it sends, and the two ceilings differ.** Each changed symbol admits up to 6 candidate sites, which are read from the repository at the reviewed revision and parsed locally, but at most 3 snippets per symbol are ever emitted into the payload (`maxPrefetchSitesPerSymbol` / `maxEmittedSitesPerSymbol` in `internal/payload/prefetch.go`). The wider admission ceiling is there so a symbol whose first matches are rejected downstream does not spend its whole allowance on them — it widens what atcr reads from your working repository, not what leaves the machine. Provider egress stays bounded by the emission cap and by `max_prefetch_bytes`.
