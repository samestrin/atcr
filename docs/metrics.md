...
| `atcr_tier4_fix_anchor_dropped_total` | counter | Individual FIX anchors dropped as imprecise (each contributed by a glued span). The set is narrowed rather than abandoned, so this counter is how a suggestion that never landed is attributed to the drop after the fact. |
