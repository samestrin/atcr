...
| Metric | Type | Description |
|--------|------|-------------|
| `atcr_tier4_index_unavailable_total` | counter | Reconcile runs where a Tier 4 symbol index was needed but was wholly or partially unavailable — the tracked tree contained no root-contained file, the eligible tracked-file count exceeded the cap (default 5000, override `ATCR_TIER4_INDEX_MAX_FILES`), the build was aborted (context cancelled), a parser failed to load for a language in the tree (that language's declarations lost), or the build completed but zero eligible files could be read. Four of those five leave no index at all, so every lookup reports "could not check" and nothing is routed to `unresolved.json`. The fifth — one language's parser failing in a tree where others loaded — leaves a working index that has simply lost that language's declarations, so lookups still resolve and route. Either way a non-zero value separates a degraded run from a clean one; read `unresolved_state` to tell which, and note it reports `unavailable` for a parser failure only when the declaration set was left empty. |
