...
| Metric | Type | Description |
|--------|------|-------------|
| `atcr_tier4_index_unavailable_total` | counter | Reconcile runs where a Tier 4 symbol index was needed but was wholly or partially unavailable — the tracked tree contained no root-contained file, the eligible tracked-file count exceeded the cap (default 5000, override `ATCR_TIER4_INDEX_MAX_FILES`), the build was aborted (context cancelled), a parser failed to load for a language in the tree (that language's declarations lost), or the build completed but zero eligible files could be read. Four of those five leave no index at all, so every lookup reports "could not check" and nothing is routed to `unresolved.json`. The fifth — a parser failing to load — leaves an index that has lost that language's declarations: lookups in it can no longer resolve (there is nothing left to point a suggestion at when the declaration set is empty), but the raw-token search still runs, so routing survives. Either way a non-zero value separates a degraded run from a clean one; read `unresolved_state` to tell which, and note it reports `unavailable` for a parser failure only when the declaration set was left empty AND the tree was read in full. A parser failure on a run that also went incomplete increments BOTH counters but reports `incomplete`, because the unread region is what withheld the verdicts and is the condition an operator can act on. |
