...
- **Opportunity-set scoping.** A record also counts only when that run was an
  *opportunity* for its lens — when some reviewer on the run raised a
  ...
  to ground a remit against, and any run whose categories are all
  non-discriminating — `other`, `out-of-scope` and `invariant` carry no topic, so
  judging on them would un-score a whole panel for a labelling failure.
  **This reduces the run count the `minRuns` floor sees**, and that floor was
  measured against unfiltered strict runs; see the known limits recorded beside
  `DefaultTrustMinRuns`.
