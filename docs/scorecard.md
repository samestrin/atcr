...
- **Opportunity-set scoping.** A record also counts only when that run was an *opportunity* for its lens — when some reviewer on the run raised a `categories_raised` value inside that lens's remit. A narrow lens is supposed to be silent most of the time, so scoring it against the full corpus makes it look like a lens that never contributes while a generalist accumulates standing by being in scope everywhere. Removing out-of-remit runs from the denominator is what makes the two rates comparable. Five kinds of record survive the opportunity gate. FOUR are never judged, and pass through untouched because nothing on the run says whether the lens's remit was in play; the fifth IS judged, on a different question, and is described after them. The four: aggregates; pre-schema-2 records (their category set is unmeasured, not empty); the registry-only lenses with no in-repo definition to ground a remit against; and any run that contributed no *discriminating* category at all. That last class has three routes, and they are indistinguishable in the store: nobody raised anything; every raised value fell outside the closed vocabulary and was dropped at the write gate (the likeliest one in practice — `reconcile/category.go` records a dry run where 72.3% of findings used a word the scorer did not recognise); or every raised value was non-discriminating, meaning `other`, `out-of-scope` or `invariant`, which carry no topic. Judging any of the three would un-score a whole panel for a labelling failure.

  The fifth is narrower, and it is judged rather than waved through — on the raised COUNT rather than on the remit. It exists to close an escape rather than to avoid one. A record that **raised findings but contributed no discriminating category of its own** passes through even when other reviewers on the same run did contribute one — it is *unscopeable*, not out of remit. Without that carve-out, bad labelling would be self-exculpating: a lens whose every `CATEGORY` word is dropped at the write gate contributes nothing to the union, so the remit test matches nothing and the record leaves the denominator entirely, taking its uncorroborated findings with it. The lens most in need of demotion would escape it on exactly the runs where it raised the most unattributable findings. The discriminator is the raised COUNT, not the category set: a lens that raised **nothing** carries no categories either, but that lens is correctly silent on an out-of-remit case and must still leave the denominator — crediting or penalising it is what opportunity scoping exists to prevent. `personas list --scores` reports the unscopeable records per lens in its `CASES` column as *unlabelled*.
