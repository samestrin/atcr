...
| `raised_includes_unresolved` | bool | conditional | `true` when `findings_raised` counts the Tier-4-routed findings (every record written from Epic 35.16.6.5 onward). Omitted on records written before it, whose denominator excluded them. `TrustPriors` uses it to avoid averaging the two definitions together — see the caution below. **Known limitation:** Epic 35.16.6.8 narrowed the denominator a second time (the `doc_shield` carve-out) without widening this flag, so a 35.16.6.5-era record and a 35.16.6.8-era record both read `true`. Where the carve-out actually fired the two are distinguishable by a nonzero `findings_doc_shielded`; where it did not fire, the two definitions produce the same number, so the blend is harmless. |
