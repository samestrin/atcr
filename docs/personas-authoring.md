...
- [ ] **Predicate-exhaustiveness rule** (built-in personas only) appears as a numbered bullet under `## Focus`, carrying BOTH anchor phrases verbatim — `enumerate every branch of that predicate and every field it is contracted to cover` and `file it on the edited branch's changed line` — with the surrounding prose in your persona's own voice, and opening on the edit trigger so the rule stays scoped to the diff. Enforced by `go test ./personas/`.
