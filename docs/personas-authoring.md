...
1. **Place the persona unit.** Move (or copy) the submitted `<name>.yaml` — and its co-located `<name>.md` prompt, if present — into `personas/community/`, **preserving any nested namespace path**: a persona named `security/owasp` lands at `personas/community/security/owasp.yaml`; do not flatten it to the repo root. Its fixture belongs in `personas/community/testdata/` (see [§3. The fixture](#3-the-fixture)).
