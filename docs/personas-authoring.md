...
- [ ] **Predicate-exhaustiveness rule** (built-in personas only) appears as a numbered bullet under `## Focus`, carrying BOTH anchor phrases verbatim — `enumerate every branch of that predicate and every field it is contracted to cover` and `file it on the edited branch's changed line` — with the surrounding prose in your persona's own voice, and opening on the edit trigger so the rule stays scoped to the diff. Test-enforced: the two anchor phrases (`TestEveryBuiltinPersona_CarriesThePredicateExhaustivenessRule`, checked as whole-file substrings) and the `ToolsEnabled`-both-ways rendering (`TestPredicateExhaustivenessRule_RendersWithToolsEitherWay`) — bullet placement, numbering, own-voice prose, and the edit-trigger opening are review conventions checked by hand at PR time, not by the suite.
