...
**Built-in personas carry the panel-wide predicate-exhaustiveness rule.** Every TOP-LEVEL `.md` file in `personas/` — the nine registered personas and `_base.md`, not `personas/community/` — must contain two anchor phrases verbatim, inside one numbered bullet under `## Focus`: `enumerate every branch of that predicate and every field it is contracted to cover` (the lens) and `file it on the edited branch's changed line` (how to file what the lens finds). Three things about how you write that bullet are load-bearing:

- **Your own voice.** Word the prose around both phrases in your persona's own voice rather than pasting another persona's sentence — a byte-identical paragraph across the panel makes reviewers converge, and correlated findings inflate the reconciler's `CONFIDENCE = HIGH` (2+ distinct reviewers) without adding independent evidence.
- **Under `## Focus`, outside the tool block.** `## Focus` sits *outside* the `{{if .ToolsEnabled}}` block; a rule placed inside it renders only for tool-using agents, and every single-shot agent silently misses it.
- **In your file, not only `_base.md`.** `_base.md` is a resolution fallback, not an inherited prefix, so a registered persona that ships its own `.md` never reads it (see the resolution chain in [registry.md](registry.md#persona-resolution-chain)).

**Why it stays a `## Focus` bullet even though the register breaks.** Bullet 6 does not read like its neighbours and that is a deliberate, recorded trade rather than an oversight. Focus bullets 1–5 run roughly 9–12 words each in the shipped files and take the shape `Label: noun, noun, noun`; bullet 6 runs several times that as a multi-clause procedure, and in a numbered list the longest, most specific entry attracts attention weight out of proportion to its rank. The alternative — compressing bullet 6 to list register and moving the procedure to a `## Predicate Exhaustiveness` section of its own — reads better and costs more than it saves: `## Focus` is the section this repo already guarantees sits outside the `{{if .ToolsEnabled}}` block, and the suite pins both anchors to a single line *within the `## Focus` span*, so a new section would have to be created, ordered, and kept outside that block in every built-in file, or the placement guard would have to fall back to the whole-file substring match it was just tightened away from. The mixed register is the accepted cost; the token cost (~90 per prompt) is negligible either way.

**Why the filing anchor exists.** The second phrase is not bookkeeping. A predicate's sibling branch is normally *unchanged* code, and the grounding gate keeps a finding only through one of six arms: the `CATEGORY out-of-scope` exemption (annotated and never promoted), a binary/mode-only change with no line data (fail-open), a file-level citation with no line on a changed file, a line within ±`groundingTolerance` of a changed range, an `EVIDENCE` match against changed text, or — for a file present ONLY because context-aware pre-fetching retrieved a snippet of it — a line inside an exact retrieved span (the `PrefetchOnly` arm, `internal/fanout/grounding.go:84-86`). That sixth arm is strictly *narrower* than the others, not an extra allowance: it deliberately bypasses both the file-level arm and the binary/mode fail-open arm, because each of those is justified by the patch having demonstrably touched the file, and for a merely-referenced file neither holds (`internal/fanout/grounding.go:66-97`). A finding whose only citation is an untouched sibling line clears none of them — so a reviewer that correctly spots the asymmetry and cites the sibling alone has its finding deleted before the report, on exactly the defect the rule exists for. Anchoring on the edited branch and quoting the sibling as `EVIDENCE` keeps it promotable. Three tests pin the rule: `TestEveryBuiltinPersona_CarriesThePredicateExhaustivenessRule` (`personas`) walks the embedded built-in filesystem and fails the suite for any built-in that omits either phrase; `TestPredicateExhaustivenessRule_RendersWithToolsEitherWay` (`personas`) fails when the rule would not render with `ToolsEnabled` both true and false; and `TestPersonaResolution_BaseOnlyTextReachesNoRegisteredAgent` (`internal/registry`) fails if `_base.md` ever becomes a shared prefix. Community personas are deliberately not walked and are exempt.
...
```markdown
6. <Predicate exhaustiveness, in your own voice — the lens anchor below is verbatim> — enumerate every branch of that predicate and every field it is contracted to cover — the contract being what the type's doc comment, the sibling arms taken together, or the predicate's tests already treat as significant, not simply every field on the struct — and report any field <in your own voice: one arm checks that a sibling arm ignores>; file it on the edited branch's changed line whenever the sibling arm is unchanged code, with CATEGORY invariant, quoting the sibling arm as evidence — the predicate arm the diff touched, not the git branch — where a predicate has more than two arms, file one finding and quote the arm with the narrowest field set; the changed-line anchor is what keeps the finding inside the scope rule below. Ignore an asymmetry the code documents as deliberate or that a branch's narrower type makes impossible, and file the rest at MEDIUM or above only when the asymmetry changes the predicate's result for some input. Enumerate using only arms visible in the payload or ones you actually read; if the sibling arms are not in front of you, report nothing rather than inferring them.
```

Everything after your own lead and report clause is shared boilerplate — carry it verbatim (only the two anchor phrases inside it are suite-pinned, but the exclusion, abstention, and cardinality clauses are panel-wide conventions the shipped files all share).
