...
- [ ] **Predicate-exhaustiveness rule** (built-in personas only) appears as a numbered bullet under `## Focus`, carrying the anchor phrase `enumerate every branch of that predicate and every field it is contracted to cover` verbatim with the surrounding prose in your persona's own voice — enforced by `go test ./personas/`.
