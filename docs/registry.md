...
The shared review settings (`payload_mode`, `review_strategy`, `on_overflow`, `max_sprint_plan_bytes`, `timeout_secs`, `payload_byte_budget`, `chunk_byte_budget`, `max_parallel`, `cache_max_bytes`, `fail_on`, `consensus`) resolve **per field, independently**, in this order:
