...
| `tool_budget_bytes` | int (per agent) | `0` (unlimited) | must be `>= 0` | Caps cumulative tool-result bytes delivered to the model across the run. `0 = unlimited`; a negative value is rejected at load. ⚠️ **The skeptic lane is the one exception to that default.** In the review, debate and executor lanes `0` really is unlimited, but `internal/verify/invoke.go` clamps this budget — including a `0` — to `EffectiveByteBudget(model, context_window_tokens, reserved_output)`, where `reserved_output` is the agent's `max_tokens` declaration floored at the built-in `8192` the review lane also floors at (an agent that declares no cap is still promised the provider's own default, so the ceiling reserves for it rather than reserving nothing) whenever the agent declares [`context_window_tokens`](#agent-fields), because unlimited is exactly the state a declared window contradicts. The clamp is a ceiling only: a smaller declared `tool_budget_bytes` still wins, an **undeclared** window leaves the value untouched, and the reservation can never itself cost the ceiling — a window too small to afford `reserved_output` derives again with nothing reserved rather than falling back to the declared value, so only a window with no input room at all (below the prompt overhead, where no reservation makes it fit) forwards the declared value instead of a derived ceiling (the engine would read a derived `0` as unlimited). Overrunning a **derived** ceiling truncates what the skeptic may still read but does NOT void its verdict — only an overrun of a budget **you** declared marks the run unverifiable. |
