...
| `max_context_lines` | *derived from the model's context window* | Per-agent cap on a single chunk's diff line count, used **only** when `review_strategy: chunked` (ignored in `bulk`). The fan-out bin-packs the persona's diff into chunks no larger than this and sends one call per chunk, so a lower value means smaller, more focused context (and more requests); a higher value packs more per call. **When unset, the budget is derived per agent from that agent's model context window** — `ChunkMaxLines` = `EffectiveByteBudget(model) / 48`, so a small-window model gets more, smaller chunks and a large-window model gets fewer, larger ones from the same diff. At the conservative `32768`-token default window this derivation yields ≈1493 lines, which is where the `1500` figure comes from: it is the *derived value at the smallest window*, not a fixed default. **Setting this field explicitly OVERRIDES that derivation** and pins every agent to a constant regardless of its model — use it only when you specifically want to defeat per-model scaling. Must be within `1..1000000`. A single file larger than this cap is sent as its own oversized chunk with a warning (a file is never split). |
