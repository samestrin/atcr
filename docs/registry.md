...
`context_window_tokens` is the window the fan-out will actually size this agent's payload against, and `window_source` names the tier it came from — `declaration` (the agent's own `context_window_tokens`), `table` (atcr's static model table), or `default` (the conservative 32,768 fallback). The table renders both as one `WINDOW` cell, e.g. `262144 (declaration)`, and appends the probed output cap when one was applied: `32768 (default) / cap 32000 (declaration)`. When review's cap differs from the probed one — doctor's `--max-tokens` caps the probe only, while the `ok_warning` hint speaks for the review run — the cell carries that operand too: `32768 (default) / cap 4096 (flag) / review cap 32000`. The caps share that cell rather than taking a column of their own because the numbers are one story — the cap is reserved out of the window when the payload is sized — and because the `ok_warning` hint tells the operator to change the cap, which a table that omitted it would name without showing.
