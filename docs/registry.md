...
| `max_prefetch_bytes` | `16384` | Byte ceiling on the **retrieved snippet bytes** in the Context Definitions block that context-aware pre-fetching prepends to every reviewer payload — the block's framing (its start/end markers and the bounded drop ledger described below) sits outside this accounting, so the rendered section can exceed the ceiling by roughly 1 KB. Pre-fetching resolves the symbols a diff changed to the **call sites that consume them**, and injects those snippets — so a defect whose mechanism lives in a file the diff never touched becomes visible to (and reportable by) a reviewer. Retrieval is structural, not semantic: it runs a single `git grep` for the changed symbol names and parses only the files that match, so it needs no index, no external service and no pre-built artifact. **`0` disables the feature outright** — no `git grep` runs and no repository source from outside the diff is sent to a provider — which is the escape hatch for operators who will not ship untouched-file contents to a third party. Negative values are rejected at load, and a negative that reaches the resolver anyway fails safe to disabled rather than unbounded. **There is deliberately no unbounded setting.** Like `max_claim_bytes` and unlike `payload_byte_budget`, `0` here does NOT mean unlimited: the injected section carries `Size 0` and is exempt from every byte budget, so this ceiling is the only thing bounding its snippet bytes. Files excluded by `.gitignore`/`.atcrignore` are never retrieved, so the payload filter governs pre-fetched context exactly as it governs the diff. When the ceiling forces a drop, up to 10 dropped snippets are **named in the section itself** (path, symbol, tier and byte count) and any remainder is disclosed as a count, rather than discarded silently. **Under `review_strategy: chunked` the block rides the first chunk only** — chunking splits the payload text on diff-file markers and the block sits above the first of them — so the agents reviewing chunks 2..N never receive it and the retrieved-span grounding widening is revoked for the whole review, recorded as `grounding_revoked` in `status.json` and warned on stderr; a chunked run that yields a single chunk falls through to the bulk path and keeps the widening (see [payload-modes.md](payload-modes.md)). Resolves at the registry and project tiers only (no CLI flag). |
