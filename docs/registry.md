...
| `max_context_lines` | *derived from the model's context window* | Per-agent cap on a single chunk's diff line count, used **only** when `review_strategy: chunked` (ignored in `bulk`). The fan-out bin-packs the persona's diff into chunks no larger than this and sends one call per chunk, so a lower value means smaller, more focused context (and more requests); a higher value packs more per call. **When unset, the budget is derived per agent from that agent's model context window** — `ChunkMaxLines` = `EffectiveByteBudget(model, declared) / 48`, so a small-window model gets more, smaller chunks and a large-window model gets fewer, larger ones from the same diff. At the conservative `32768`-token default window this derivation yields ≈1493 lines, which is where the `1500` figure comes from: it is the *derived value at the smallest window*, not a fixed default. Declaring [`context_window_tokens`](#agent-fields) raises the window that derivation reads, and so raises this budget (a `128000` declaration yields ≈8437 lines). **Setting this field explicitly OVERRIDES that derivation** and pins every agent to a constant regardless of its model — use it only when you specifically want to defeat per-model scaling. It overrides **only the per-chunk line budget under `review_strategy: chunked`** — a `context_window_tokens` declaration still drives the payload byte budget, the bulk fall-through, the fallback budget, and the recorded sizing, so the two fields do not cancel each other out. (In the default `bulk` strategy this field is ignored entirely, so there it overrides nothing at all.) Must be within `1..1000000`. The cap is measured against a chunk's **delivered** line count — every line dispatched, engine-rendered preamble included — not against the named file's own diff. A single file larger than this cap is still sent as its own oversized chunk with a warning (a file is never split), but the same warning also fires for a file comfortably INSIDE the cap when preamble lines push the delivered total over it. The warning names those lines separately (`plus N engine-rendered preamble line(s)`); on a range review they are the claim ledger, and the lever that shrinks them is `max_claim_bytes` — a run setting in the [`.atcr/config.yaml`](#atcrconfigyaml-project-level) table, not an agent field — and **not** this field. On a range review there is a **second** preamble section with its own, larger default: the pre-fetched Context Definitions block, whose lever is `max_prefetch_bytes` (`16384` by default, `0` to disable). When the warning reports preamble lines, check both keys — tuning only `max_claim_bytes` can leave the larger of the two sections untouched. |
| `context_window_tokens` | *unset — resolved from a static model table, else `32768`* | Declares **this agent's model context window in tokens**, the value every payload budget is derived from. Resolution order is **agent declaration → static model table → `32768` default**; a declaration wins over the table because the table keys on provider-qualified ids (`deepseek/deepseek-v4-pro`) and is a conservative floor, while a declaration is machine-specific truth about how *your* proxy serves *that* deployment. Declare it when your roster reaches models through bare proxy aliases (`glm-5.2`, `kimi-k3`) that match no table key and therefore silently resolve the conservative default — the under-fill can be 30×. ⚠️ **On the default `bulk` strategy, `payload_byte_budget` caps how much of that window is usable.** The applied bulk budget is `min(per-agent budget, payload_byte_budget)`, and `atcr init` writes `payload_byte_budget: 524288` into every project config, so declarations above roughly `162000` tokens buy no additional payload on bulk — the extra window is silently unused. Raise `payload_byte_budget` alongside the declaration if you want the full window on bulk. `review_strategy: chunked` derives a per-chunk **line** budget from the declaration and clamps that to `chunk_byte_budget` (which inherits `payload_byte_budget` when unset), so the same ceiling applies on both strategies unless you set the chunk budget separately — a large declaration means fewer, larger chunks, never a chunk bigger than the budget you set. An explicit `max_context_lines` overrides the derived budget and is used verbatim, clamp included. There is no runtime probing and **no CLI override**; capability is registry-declared, exactly like `supports_function_calling`. Must be within `1..10000000`. ⚠️ **Declare `min(model window, proxy ceiling)`, not the model's published spec.** A request-body guardrail in front of the model (a PII scanner, a size limit) can cap the usable window far below the model's own, and the failure surfaces as an opaque HTTP 400 with no hint that the payload was simply too large. ⚠️ Leaving an agent undeclared is the safe choice — it keeps today's conservative default rather than guessing. **Scope:** consumed by the review fan-out for payload sizing. The **skeptic** lane (`internal/verify/invoke.go`) builds no sizing record either, but it does use the declaration for the one budget it can bound: the tool-output ceiling. `tool_budget_bytes` is a flat per-agent number with no relation to the model behind it, so a skeptic reading files through the tool loop could be walked past a small window by a few large reads, so a declared window clamps that budget. The clamp is a ceiling only — a smaller `tool_budget_bytes` still wins — and an **undeclared** agent is left exactly as it was. The derivation, its output reservation and what an overrun means are stated once, in the [`tool_budget_bytes`](#tool-using-reviewer-fields-active-in-20) row; this row does not restate them, because two descriptions of one clamp are how the two drifted apart. The **executor** and **debate** lanes still derive nothing from it. **Note:** a YAML float (`1.28e6`, `128000.5`) is **rejected at load**, naming the offending key — registry-wide behavior for every **integer-typed** field, not specific to this one. (It used to be silently truncated, so `1.28e6` loaded as `1280000` and `1200.5` as `1200`; that was fixed rather than documented.) Genuinely float-typed fields are unaffected: `temperature` is a float, and `payload_escalation.churn_ratio` is a float with its own NaN/Inf/range guard. |
| `max_tokens` | *unset — the fan-out's built-in 8192* | Declares **this agent's output-token cap**, resolved as `--max-tokens` flag → this declaration → the built-in default. It exists because one hardcoded cap cannot serve both model classes: a reasoning/thinking model spends output budget on chain-of-thought before emitting visible content, so a cap sized for ordinary reviewers makes it finish mid-reasoning — the response hits `finish_reason=length` with zero parsed findings, the truncation gate demotes the slot, and **the lens contributes nothing while the run still reports success**. `atcr doctor` has always hinted "raise `--max-tokens`" for that symptom; `atcr review` now has the flag. ⚠️ The cap is **also reserved out of the context window when sizing the payload**, so raising it narrows that agent's input budget by the same amount — those tokens are genuinely spoken for. Must be within `1..1000000`. **Scope:** consumed by the review fan-out, the skeptic (`internal/verify/invoke.go`) and the debate lane (`internal/debate/protocol.go`). Those two lanes forward the **declaration only** — they have no `--max-tokens` flag to read and impose no built-in default, so an agent that declares nothing keeps its provider's own default there rather than the fan-out's `8192`. The **executor** (`internal/verify/executor.go`) is the one lane still uncovered, and for a different reason: `ExecutorConfig` has no `max_tokens` field at all, so there is no declaration to forward — adding one is a registry change, not a wiring fix. |
...
| `on_overflow` | `chunk` | Degradation policy when a payload exceeds the model's effective byte budget: `chunk` (default — split the diff into window-sized chunks, preserving all files), `truncate` (drop lowest-priority files and record the truncation), `fallback` (recognized but not yet dispatched; requires fallback-provenance recording), or `fail` (hard-fail the slot). Resolves at the registry and project tiers only (no CLI flag). **`fail` and `fallback` hard-fail before dispatch in four cases:** a payload that overflows the agent's own budget; an agent under `review_strategy: bulk` whose resolved window funds no input budget at all (there the `chunk`/`truncate` arms ship only the smallest file instead); that same zero-budget agent under `review_strategy: chunked`, where there is **no** smallest-file arm at all — the `chunk`/`truncate` arms chunk at the `minChunkLines` floor, shipping chunks the window provably cannot hold, so the policy is consulted on that path too rather than degrading silently; and a **fallback** whose own budget cannot hold the prompt its primary was sized for. The zero-budget refusal names the agent, its resolved window, the output cap, and both declarations that can close the budget (`max_tokens`, `context_window_tokens`) — on a `--resume` that message is the only signal, since the zero-budget stderr warning is suppressed on the resume rebuild path. That last case is measured against the payload actually shipped, not against the two budgets alone, so an ordinary small diff with a smaller-windowed backup does not trip it. **`truncate` re-fits an overflowing fallback** rather than shipping a prompt it cannot hold: the fallback re-packs the payload against its OWN budget, keeping the single smallest file when nothing else fits (never an empty review), and records its own `truncation`, `effective_budget`, `chunk_count: 1` and `degradation_action: truncate` — or `degradation_action: overflow`, with the stderr warning still emitted, when even the smallest single-file framing exceeds the fallback's budget (a backup too small for the lane at any framing). On that arm `effective_budget` still reports the budget the payload was sized to, which is a normal positive number in the common case — so a present `effective_budget` does **not** rule out the overflow arm; read `degradation_action` for that. It reads 0 (and is omitted) when the applied budget is exhausted: a window that funds no input budget at all, or one whose remaining budget is consumed by `payload_byte_budget` or by the injected `--sprint-plan` SCOPE CONSTRAINT block. Baseline coverage then credits only the files that fallback actually reviewed, so a shed file is re-scanned rather than recorded as reviewed. The re-fit needs a per-file payload to re-pack, which `review_strategy: chunked` diff chunks do not have (they are split by text on diff boundaries) — those, and `chunk` on every path, still warn on stderr and ship a degraded payload recorded as `degradation_action: overflow`. |
| `max_sprint_plan_bytes` | `65536` | Byte ceiling for the `--sprint-plan` file's SCOPE CONSTRAINT block. The plan body is truncated to this size before injection; `0` and negative values are rejected. Resolves at the registry and project tiers only (no CLI flag). **It is a ceiling, not the cap.** The plan is funded at **one eighth** of whatever byte budget sizes the call it rides, so the effective cap is `min(max_sprint_plan_bytes, budget/8)` — `budget` being `payload_byte_budget` at the run tier, the agent's own effective budget per agent, and `min(agent budget, chunk_byte_budget)` on a `review_strategy: chunked` run. A budget under 8 bytes funds zero plan bytes and the block is **dropped**, and the review runs unscoped. |
| `max_claim_bytes` | `8192` | Byte ceiling on the COMMIT-MESSAGE text read into the claim ledger (the `## CLAIMS TO VERIFY` block prepended to every reviewer payload). **`0` disables the feature outright** — no `git log` runs and no commit text is sent to a provider — which is the escape hatch for operators who will not ship commit prose to a third party. Negative values are rejected at load, and a negative that reaches the resolver anyway fails safe to disabled rather than unbounded. **There is deliberately no unbounded setting.** Unlike `payload_byte_budget` and `cache_max_bytes`, `0` here does NOT mean unlimited: the ledger entry carries `Size 0`, so on the ordinary shed its bytes are invisible to `payload_byte_budget`, to each agent's own effective budget, and therefore to the `on_overflow: fail` gate. (Two bounds exist but neither caps the ledger's SIZE: the shed exemption applies only while the ledger fits the budget, and the fallback re-fit re-sizes every entry to its dispatched bytes and so counts the ledger like any other entry.) This ceiling is the only thing bounding them. Resolves at the registry and project tiers only (no CLI flag). |
| `max_prefetch_bytes` | `16384` | Byte ceiling on the **entire rendered Context Definitions section** that context-aware pre-fetching prepends to every reviewer payload — its start/end markers, the untrusted-content notice, the retrieved snippets and the bounded drop ledger described below are all billed inside this accounting, so the emitted section never exceeds the ceiling. Pre-fetching resolves the symbols a diff changed to the **call sites that consume them**, and injects those snippets — so a defect whose mechanism lives in a file the diff never touched becomes visible to (and reportable by) a reviewer. Retrieval is structural, not semantic: it runs a single `git grep` for the changed symbol names and parses only the files that match, so it needs no index, no external service and no pre-built artifact. **`0` disables the feature outright** — no `git grep` runs and no repository source from outside the diff is sent to a provider — which is the escape hatch for operators who will not ship untouched-file contents to a third party. Negative values are rejected at load, and a negative that reaches the resolver anyway fails safe to disabled rather than unbounded. **There is deliberately no unbounded setting.** Like `max_claim_bytes` and unlike `payload_byte_budget`, `0` here does NOT mean unlimited: the injected section carries `Size 0`, so it is uncounted on the **ordinary** shed — but not exempt from every budget. On the fallback re-fit every entry is re-sized to its body, and the cumulative exempt funding then keeps a section only while it still fits; the Context Definitions block carries the lowest exempt rank (below the claim ledger), so it is the first exempt section to lose its funding. This ceiling is the only thing bounding its bytes. Files excluded by `.gitignore`/`.atcrignore` are never retrieved, so the payload filter governs pre-fetched context exactly as it governs the diff. When the ceiling forces a drop, up to 10 dropped snippets are **named in the section itself** (path, symbol, tier and byte count) and any remainder is disclosed as a count, rather than discarded silently. **Under `review_strategy: chunked` the block rides the first chunk only** — chunking splits the payload text on diff-file markers and the block sits above the first of them — so the agents reviewing chunks 2..N never receive it and the retrieved-span grounding widening is revoked for the whole review, recorded as `grounding_revoked` in `status.json` and warned on stderr; a chunked run that yields a single chunk falls through to the bulk path and keeps the widening (see [payload-modes.md](payload-modes.md)). Resolves at the registry and project tiers only (no CLI flag). |
...
| `chunk_byte_budget` | *unset — inherits `payload_byte_budget`* | Byte budget used **only** to size a per-chunk payload under `review_strategy: chunked`, via `ClampLinesToByteBudget`. It exists because `payload_byte_budget` was doing two jobs that want opposite values: it caps the concatenated payload (dropping whole files largest-first) **and** derived the per-chunk line budget. Lowering it to fit a small-window model therefore shed files no reviewer ever saw; raising it to keep those files grew every agent's chunks. Left unset it resolves to whatever `payload_byte_budget` resolved to — including a `--byte-budget` override — so a config that never mentions it is sized exactly as before. Set it to hold a large payload while still chunking small. `0` = unlimited; negative is rejected. Resolves at the registry and project tiers only (no CLI flag). **It also caps the injected `--sprint-plan` block**, not only the per-chunk payload: on a chunked run the plan is funded at `min(agent budget, chunk_byte_budget)/8`, so the effective ceiling becomes `min(max_sprint_plan_bytes, chunk_byte_budget/8, agent budget/8)`. Setting it to the `65536` this key's own generated-config example suggests therefore shrinks an injected plan from up to 64 KiB to 8 KiB. Only a total **drop** (a value in `1..7`) is warned about on stderr; a shrink is silent. **Not consulted on a baseline scan.** `atcr review --all` / `--dir` splits each agent's file set into per-agent chunks *unconditionally* — a separate mechanism from `review_strategy: chunked`, which is gated off for baseline scans entirely. That partition is sized by the agent's effective budget capped by `payload_byte_budget`, so lowering `payload_byte_budget` still shrinks baseline chunks (and sheds files) no matter what this key is set to. |
...
| `max_tokens` (per-agent) | `--max-tokens` |
| `fail_on` | `--fail-on` |
| `max_parallel` | `--max-parallel` |

`--timeout` takes bare seconds (no unit suffix, matching `timeout_secs`).
`review_strategy`, `on_overflow`, `max_sprint_plan_bytes`, `max_claim_bytes`, `max_prefetch_bytes`, and `cache_max_bytes` resolve at the registry and project tiers only — there is no CLI flag for them.
...
The shared review settings (`payload_mode`, `review_strategy`, `on_overflow`, `max_sprint_plan_bytes`, `max_claim_bytes`, `max_prefetch_bytes`, `timeout_secs`, `payload_byte_budget`, `chunk_byte_budget`, `max_parallel`, `cache_max_bytes`, `fail_on`, `consensus`) resolve **per field, independently**, in this order:
...
| `tool_budget_bytes` | int (per agent) | `0` (unlimited) | must be `>= 0` | Caps cumulative tool-result bytes delivered to the model across the run. `0 = unlimited`; a negative value is rejected at load. (That validation governs registry files; a programmatically built `AgentConfig` bypasses it, and the skeptic lane never forwards the negative — a declared window derives a real ceiling or the 1-byte floor, and with no window a negative gets the same 1-byte floor, so the engine never receives the value as unlimited.) ⚠️ **The skeptic lane is the one exception to that default.** In the review, debate and executor lanes `0` really is unlimited, but `internal/verify/invoke.go` clamps this budget — including a `0` — to `EffectiveByteBudget(model, context_window_tokens, reserved_output)`, where `reserved_output` is the agent's `max_tokens` declaration when it makes one, else the built-in `8192` (a DEFAULT, not a floor — an agent declaring `max_tokens: 100` really does reserve `100`; one declaring nothing is still promised the provider's own default, so the ceiling reserves the built-in rather than reserving nothing), capped at **half the window's input room** so the ceiling leaves room for the reply — except where the window's whole input room is one token, which no reservation can share — whenever the agent declares [`context_window_tokens`](#agent-fields), because unlimited is exactly the state a declared window contradicts. The clamp is a ceiling only: a smaller declared `tool_budget_bytes` still wins, an **undeclared** window leaves a non-negative value untouched, and the reservation can never itself cost the ceiling — the claim is always at most half the input room rather than the full `reserved_output`, which is what keeps the derivation continuous (no window derives a smaller ceiling than a smaller window does). The cap binds below `2 * reserved_output + 4096` tokens (`20480` at the built-in default) — including windows that could have funded the full reservation, which below that boundary cannot also leave a usable read. So in that band `reserved_output` is no longer an upper bound on the reply: a reply that spends its whole cap can still overshoot the window. Every window at or above the boundary reserves the full cap and cannot. Only a window with no input room at all (at or below the prompt overhead, where no reservation makes it fit) has no ceiling to derive: there a **positive** declared `tool_budget_bytes` stands, while a `0` becomes a 1-byte floor, because a forwarded `0` is what the engine reads as unlimited. Overrunning a **derived** ceiling truncates what the skeptic may still read but does NOT void its verdict — only a declaration BELOW the derived ceiling is ever the number enforced, so a larger one's overrun is a derived trip and keeps the verdict. The **1-byte floor is the one exception**: it is not a declaration and not a real derived ceiling, it trips on the first tool result by construction, and its trip DOES mark the run `unverifiable` — a window that cannot hold one tool result cannot fund a trustworthy verification, and saying so is the point of the floor. |
...
atcr doctor --agents bruce,kai   # test only these listed agents (their fallback chains are still endpoint-probed)
...
Flags: `--max-tokens` (default `8192`), `--timeout` (default `60`s), `--json`, `--agents <a,b,...>` (restrict to a subset of the directly-listed agents — each selected agent's fallback chain is still endpoint-probed so its health verdict stays accurate; the persona scan never walks the chain). The token budget defaults high on purpose: reasoning/thinking models spend completion tokens on internal reasoning, so a small budget can exhaust before the marker is emitted. It is deliberately the **same** default the review fan-out applies, so an agent that declares no `max_tokens` is probed at the cap `atcr review` will actually use — a probe is only evidence about the invocation it reproduces. Setting `--max-tokens` explicitly also overrides every declaration, so agents sharing an endpoint then collapse to a single probe instead of one per declared cap.

Doctor also reports panel-composition gaps: the built-in persona class guard walks embedded built-ins only, so an agent whose persona is a community (or hand-edited project) prompt can lack the panel-wide predicate-exhaustiveness rule with nothing failing anywhere. When a **directly-listed** agent — one named in `agents` or `serial_agents` — resolves to such a prompt, doctor prints a stderr warning naming the agents, and `--json` carries them as `predicate_rule_gaps`. This is advisory — it does not change the exit code — but it is the signal that one panel member will not hunt the defect class the rest of the panel was tuned for.

The persona scan is deliberately narrower than the effective roster: it does **not** walk `fallback` chains. A fallback's own persona never renders, because `atcr review` resolves a chain's persona by the **primary's** name and passes it to every fallback — so naming a `-backup` entry's rule gap would send you to edit a file that changes nothing.

A persona that cannot be **resolved at all** is reported separately, as `persona_resolution_errors` in `--json` and as its own stderr line. It is not a rule gap: a prompt nobody could read supports no verdict about which rules it carries. Unlike `predicate_rule_gaps`, **doctor exits non-zero on it** — `atcr review` resolves the same personas and hard-fails the run, so such an agent has no working invocation path, which is exactly what this command's exit code reports. That matters most under `--json`, which prints no human warning: the exit code is the only signal a CI gate gets.
...
| `ok_warning` (no input budget) | The endpoint is healthy — **this class fires whether or not the marker came back**, and when it did not, the row says so and this remedy replaces the marker-absent one above — but the resolved window leaves no input budget once the output cap and the fixed prompt overhead are reserved — so `atcr review` would ship only the smallest single file, or refuse the run outright under `on_overflow` `fail`/`fallback`. The cap in the hint is the one **`atcr review`** resolves — the agent's own `max_tokens` declaration, else the built-in default `8192` — which may differ from the cap this probe used. The two **differ** whenever `atcr doctor --max-tokens N` is passed with an `N` that is not the cap review would resolve for that agent; unflagged they are always the same value, and they also coincide when `N` equals the agent's own declaration (or `8192` for an agent that declares none). When they differ the hint's number can disagree with the row's own `max_tokens` column; act on the hint's. Read the row's `context_window_tokens` against that number. | **Do not raise the cap** — it is reserved out of this same window, so raising it deepens the problem while the probe still passes (the marker prompt is trivial at any cap). Instead: lower its `max_tokens`, or raise (or drop) its `context_window_tokens` declaration. This verdict fires **regardless** of `atcr doctor --max-tokens`: doctor's flag is not in review's resolution order, so it neither triggers the warning nor suppresses it, and an agent whose own `max_tokens` declaration closes its budget is still reported on a flagged run. |
...
`context_window_tokens` is the window the fan-out will actually size this agent's payload against, and `window_source` names the tier it came from — `declaration` (the agent's own `context_window_tokens`), `table` (atcr's static model table), or `default` (the conservative 32,768 fallback). The table renders both as one `WINDOW` cell, e.g. `262144 (declaration)`, and appends the probed output cap when one was applied: `32768 (default) / cap 32000 (declaration)`. When review's cap differs from the probed one — doctor's `--max-tokens` caps the probe only, while the `ok_warning` hint speaks for the review run — the cell carries that operand too: `32768 (default) / cap 4096 (flag) / review cap 32000`. When the probe applied no cap at all but the row is still healthy enough to carry a hint — a firing zero-budget verdict is `ok_warning` — the review operand appears without a probed cap beside it: `32768 (default) / review cap 8192`. A row that placed **no call** (`missing_key`, `invalid_config`) carries neither cap, so the two-part form is never a cap the probe used. The caps share that cell rather than taking a column of their own because the numbers are one story — the cap is reserved out of the window when the payload is sized — and because the `ok_warning` hint tells the operator to change the cap, which a table that omitted it would name without showing.

`max_tokens` is the output cap the probe actually ran at, after the same `--max-tokens` → agent declaration → unset resolution the probe applies. It is reported for the same reason the window is: the cap is applied silently, so an `ok_warning` row is otherwise unreadable — an operator cannot tell whether the probe ran at the agent's own declaration or at doctor's default, and that is exactly what decides whether raising the declaration would change anything.
...
`review_max_tokens` is the cap `atcr review` will resolve for this agent absent a review-side `--max-tokens` — the agent's own declaration, or the shared default when undeclared. It is the operand the zero-budget verdict and its `ok_warning` hint actually speak for: doctor's `--max-tokens` caps the probe only, so under the flag `max_tokens` and `review_max_tokens` genuinely differ, and a budget re-derived from `context_window_tokens` and `max_tokens` alone would disagree with the hint atcr shipped. The budget itself is `context_window_tokens − review_max_tokens − 4096`, where **4096** is atcr's fixed prompt overhead (the constant the prose elsewhere calls "the fixed prompt overhead"); all three operands are needed, so a consumer re-deriving from two of them will not reproduce the number. Always present, like `max_tokens` — a `0` or absent value means the report predates this field, never that the cap is zero: `Run` floors it at the shared default and cannot emit `0`.
