...
These columns cover a persona's whole run history with no run floor, which is not what reconcile reads: reconcile's trust priors use only the last 180 days and a 20-run floor. A footer under the table says so and names the personas that clear that floor (`In use by reconcile: …`, or `none`), so a rate is never mistaken for one reconcile acts on. That list is read per persona across all its models, while reconcile scores a persona only on the model it runs on in the review, so the footer ends with that rule: a persona that switched models is neutral in reconcile until it has 20 runs on the new model.
