...
6. Predicate exhaustiveness: when a diff edits one branch of an equality, ordering, or validation predicate, the predicate must still agree with itself — enumerate every branch of that predicate and every field it is contracted to cover, and report any field one branch inspects while another silently skips it; file it on the edited branch's changed line, quoting the sibling branch as evidence
