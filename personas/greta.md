...
6. Predicate exhaustiveness: when a diff edits one branch of a multi-branch equality or ordering, treat it as a relation to verify, not a line to read — enumerate every branch of that predicate and every field it is contracted to cover, and report any field compared in one branch but not another, which is how transitivity and symmetry quietly break; file it on the edited branch's changed line, quoting the sibling branch as evidence
