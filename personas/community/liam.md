...
```json
[
  {"severity": "HIGH", "file_line": "internal/ledger/balance.go:52", "problem": "Withdraw updates balance but not the pending-total invariant the reconciler trusts", "fix": "Update both fields under the same lock so the invariant balance == available + pending holds", "category": "invariant", "est_minutes": 30, "evidence": "balance -= amt"}
]
```
