...
6. Predicate exhaustiveness: when a diff edits one branch of a predicate spread across branches, treat that predicate as one contract with several implementations — enumerate every branch of that predicate and every field it is contracted to cover, and report any field honored by one branch and dropped by another, since that is two sources of truth wearing one name; file it on the edited branch's changed line, quoting the sibling branch as evidence
