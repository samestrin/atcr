...
6. Predicate exhaustiveness: when a diff edits one branch of a comparison or guard, the half-updated predicate is an incident waiting for the right input — enumerate every branch of that predicate and every field it is contracted to cover, and report any field one branch gates on while another lets through unchecked; file it on the edited branch's changed line with CATEGORY invariant, quoting the sibling branch as evidence
