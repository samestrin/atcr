...
func TestParseSelfDecline(t *testing.T) {
	...
	tests := []struct {
		...
		{"marker with space separator declines", "ATCR_DECLINE requires a cross-module refactor", true},
		{"marker with newline separator declines", "ATCR_DECLINE\ntoo complex", true},
		{"quoted marker declines", `"ATCR_DECLINE: too hard"`, true},
		{"quoted marker literal declines", "'ATCR_DECLINE'", true},
		{"nested quote pairs do not decline", "''ATCR_DECLINE''", false},
		{"mismatched quote pair does not decline", `"ATCR_DECLINE'`, false},
