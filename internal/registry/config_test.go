...
func TestAgentConfig_LanguageField_Canonicalization(t *testing.T) {
	r := &Registry{Agents: map[string]AgentConfig{
		"a": {Provider: "p", Model: "m", Language: []string{".Go", " .TS ", "GO", " .Ü ", "..go"}},
	}}
	r.applyDefaults()
	assert.Equal(t, []string{"go", "ts", "go", "ü", "go"}, r.Agents["a"].Language)

	r.applyDefaults()
	assert.Equal(t, []string{"go", "ts", "go", "ü", "go"}, r.Agents["a"].Language, "canonicalization must be idempotent")
}
