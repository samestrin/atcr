...
var allowedInternalImports = map[string][]string{
	...
	// and atomicfs (crash-safe backup + atomic write) as its only internal deps
	// (Phase 2); revert.go builds on the same two when it lands (Phase 3).
	"autofix": {"payload", "atomicfs"},
