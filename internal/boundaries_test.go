...
var allowedInternalImports = map[string][]string{
	...
	"payload":        {"gitrange", "atomicfs", "log", "gitexec", "stream", "cache"}, // log: single diagnostic sink, injected via context (epic 4.0 phase 4.1); gitexec: hardened git diff/show subprocess (sprint 32.4); stream: BuildFileIndex git ls-files tracked-file enumeration for the --all/--dir baseline scan walker (fullrepo.go, sprint 35.0); cache: HashText "sha256:<hex>" digest for the incremental file-hash skip (fullrepo.go/filehash.go, sprint 35.0 — format reuse only, NOT cache.Store)
