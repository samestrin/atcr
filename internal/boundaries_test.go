...
var allowedInternalImports = map[string][]string{
	...
	"timewindow":      {},                                                                                                                                                   // the one grammar behind every --since-style window flag (Nd/Nw/Nm plus "all"); stdlib-only leaf so both history and scorecard can share it without either depending on the other
	"history":         {"stream", "timewindow"},                                                                                                                             // Epic 19.0: parses pool findings.txt via stream.ParseSource to persist the append-only finding-history ledger; stream is the zero-dependency findings leaf
	"localdebt":       {"history", "reconcile", "fanout"},                                                                                                                   // Epic 20.1: .atcr/debt/ append-only public TD store; imports history for FindingID identity only (not its .planning/-scoped storage), mirrors the scorecard ledger mechanics. reconcile + fanout (sprint 35.13 T6): PersistForReconcile is the ONE persistence bridge both `atcr reconcile` and the MCP atcr_reconcile handler call, so it consumes reconcile.Result and fanout.ReadPoolSummary/ReadManifest directly — the exact shape and rationale of scorecard's own {llmclient, reconcile, fanout, version} entry below. Acyclic: neither reconcile nor fanout imports localdebt. Duplicating the bridge into internal/mcp instead would let the two entry points diverge, which is the defect this closes
	...
	"payload":         {"gitrange", "atomicfs", "log", "gitexec", "stream", "cache", "astgroup"},                                                                            // log: single diagnostic sink, injected via context (epic 4.0 phase 4.1); gitexec: hardened git diff/show subprocess (sprint 32.4); stream: BuildFileIndex git ls-files tracked-file enumeration for the --all/--dir baseline scan walker (fullrepo.go, sprint 35.0); cache: HashText "sha256:<hex>" digest for the incremental file-hash skip (fullrepo.go/filehash.go, sprint 35.0 — format reuse only, NOT cache.Store); astgroup: HEAD-tree skeleton extraction + McCabe score for per-file payload escalation (skeleton.go, epic 35.1) — astgroup imports no internal package, so the edge is strictly downward and cycle-free, and reconcile already depends on it for the same reason
	"llmclient":       {"registry", "errors", "circuitbreaker"},                                                                                                             // circuitbreaker: per-provider fail-fast on the API call path (epic 4.5)
	"hookobs":         {"llmclient", "circuitbreaker"},                                                                                                                      // Epic 35.0: model-invocation observation seam. Sits below cli/fanout/verify/debate/mcp so each can wrap its client or stamp call identity; It imports exactly two packages: llmclient for the call signatures it decorates, and circuitbreaker for the logical provider name it reads off the context. Imports nothing above llmclient/circuitbreaker, so any current or future call site can use it without a cycle.
	"doctor":          {"llmclient", "registry", "payload"},                                                                                                                 // payload: ResolveContextWindow is the single source of truth for the declaration -> static table -> default chain, so the pre-flight WINDOW column reports the window the review will ACTUALLY size against (epic 35.16.5.1 TD). Re-deriving it here would let the diagnostic and the behavior it diagnoses drift, which is the whole failure the column exists to catch. Strictly downward and cycle-free: payload imports no package above it and never imports doctor
	"fanout":          {"llmclient", "registry", "stream", "payload", "tools", "log", "metrics", "circuitbreaker", "validation", "atomicfs", "cache", "gitexec", "hookobs"}, // hookobs: attaches the agent identity to the call context at the same chokepoint as the circuit-breaker provider, so an audit observer can attribute an invocation to a persona (epic 35.0); gitexec: hardened `git rev-parse` in resolveHeadSHA (sprint 32.4); // log: WithAgent per-agent correlation (epic 4.0 phase 4.2); metrics: fan-out instrumentation (epic 4.4); circuitbreaker: provider threaded onto the call context (epic 4.5); validation: engine-level --output-dir system-path reject for non-CLI callers (stdlib-only leaf); atomicfs: CopyPath for the EXDEV copy-fallback in backupExisting's crash-safe swap, the shared low-level fs leaf reconcile/verify already import (epic 4.7.1); cache: diff-cache replay on the single-shot review path (epic 5.2)
	"reconcile":       {"stream", "atomicfs", "astgroup", "metrics"},                                                                                                        // astgroup: AST-isomorphism grouper wired as the primary clustering signal (epic 13.1); metrics: observability counter for a Tier 4 symbol index that could not be built (epic 35.16.6.5) — the SAME case stream carries metrics for, one layer down: when Tier 4 is silently off every lookup reports "could not check", nothing is ever routed to the sidecar, and the run is indistinguishable from a healthy one. metrics is a stdlib-only leaf importing no internal package, so the edge is strictly downward and cycle-free
	"scorecard":       {"llmclient", "reconcile", "fanout", "version", "timewindow"},                                                                                        // version: atcr_version stamped into the public submission envelope (epic 10.0)
	"telemetrytest":   {"telemetry"},                                                                                                                                        // Epic 35.12 TD: shared test-suite hermeticity guard (consent-env pin + transport interceptor); an ordinary package because a _test.go symbol cannot cross a package boundary, so cli and cmd/atcr can both import it. Imported only by _test.go files, never linked into the shipped binary.
	"telemetry":       {"log", "version"},                                                                                                                                   // Sprint 28.0: fire-and-forget usage-ping client; log: debug-level swallowed-failure logging via context (stdlib net/http otherwise)
	...
	"verify":          {"reconcile", "stream", "registry", "fanout", "payload", "tools", "llmclient", "hookobs", "atomicfs", "atomicwrite", "log", "sandbox", "repro"},                        // hookobs: skeptic + fix-executor clients are observed for the enterprise audit trail (epic 35.0); log: skeptic-failure routing (epic 4.0 phase 4.2); atomicwrite: shared group-write helper; sandbox: --exec backend resolution + dispatcher wiring (epic 11.0); repro: evidence_exec write-back stamping (epic 11.0)
	"debate":          {"reconcile", "stream", "registry", "fanout", "payload", "tools", "llmclient", "hookobs", "atomicfs", "atomicwrite", "log"},                                            // hookobs: seat clients are observed for the enterprise audit trail (epic 35.0); cross-examination stage; mirrors verify's harness; atomicwrite shared group-write helper (epic 6.0)
	"mcp":             {"gitrange", "payload", "registry", "llmclient", "fanout", "stream", "reconcile", "report", "verify", "debate", "scorecard", "localdebt", "log", "metrics", "hookobs"}, // localdebt (sprint 35.13 T6): handleReconcile persists through the same localdebt.PersistForReconcile bridge the CLI calls, closing the MCP-never-persists boundary (TD-002); mirrors the scorecard edge immediately above it in the handler
	"benchmarkimport": {"benchmark", "gitexec"},                                                                                                                                               // authoring-time aacr-bench → standard-v1 ingestion; benchmark for the Manifest/Case contract it emits against, gitexec for the hardened clone fallback (epic 35.16.2)
	"benchmark":       {"scorecard", "version", "gitexec"},                                                                                                                                    // standard-suite contract + suite-tagged submission envelope; reuses scorecard.PublicRecord for one public reviewer schema, version for atcr_version (epic 10.0); gitexec: repo-state-v1 materializes each case into a real git repository so the review RANGE — and with it the claim ledger and pre-fetching this tier measures — exists at all (epic 35.16.10), the same hardened-git edge benchmarkimport already carries
