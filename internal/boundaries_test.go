...
var allowedInternalImports = map[string][]string{
	...
	"hookobs":        {"llmclient", "circuitbreaker"},            // Epic 35.0: model-invocation observation seam. Sits below cli/verify/debate so all three can wrap their clients; llmclient for the call signatures it decorates, circuitbreaker for the logical provider name on the context. Imports no higher layer — it must stay a leaf so any future call site can use it.
