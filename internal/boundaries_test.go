...
var allowedInternalImports = map[string][]string{
	...
	// autofix currently holds only the retained go-gitdiff behavioral contract
	// test (Sprint 17.0 Phase 1). The dependency-direction walk skips _test.go,
	// so this entry exists to satisfy the allowlist-completeness check; Phase 2's
	// apply.go/revert.go will expand it (atomicfs, payload) when it lands.
	"autofix": {},
