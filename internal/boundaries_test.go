...
var allowedInternalImports = map[string][]string{
	...
	"hookobs":        {"llmclient", "circuitbreaker"},            // Epic 35.0: model-invocation observation seam. Sits below cli/fanout/verify/debate/mcp so each can wrap its client or stamp call identity; llmclient for the call signatures it decorates, circuitbreaker for the logical provider name on the context. Imports nothing above llmclient/circuitbreaker, so any current or future call site can use it without a cycle.
