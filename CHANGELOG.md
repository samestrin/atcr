...
- `truncated` on a finding's `verification` block (additive, `omitempty`, same shape as `challenge_survived`). A verdict reached from a shortened read now reaches the artifacts that read verdicts: `report.md` appends `(answered from a truncated read)` — on the main finding line AND in the collapsed Refuted section, which is where it matters most, since refuted is the verdict the gate excludes — and the reviewer's durable `survived_skeptic_rate` leaves such a verdict out of the ratio entirely, neither numerator nor denominator, rather than charging a partial read as a full-confidence one. Previously the fact reached `reconciled/verification.json` alone. **The report reads `truncated`; the score reads `trippedBudgets` in `reconciled/verification.json`** — the scorecard is emitted during `atcr reconcile`, which rebuilds `findings.json` from `sources/` and strips its `verification` blocks, so `verification.json` is the only verdict record still standing then. `atcr debate` now keeps the two in step: a ruling that clears the caveat also clears the matching `tool_budget_bytes` entry, in the same atomic write, so the report and the score can no longer disagree about the same verdict. **When EVERY verdict in a run is truncated the key is OMITTED, not published as `0.0`** — an all-truncated run leaves nothing to divide, and a published `0.0` is indistinguishable from a reviewer whose findings were all refuted, which is the strongest negative signal the metric carries. The same omission covers a reviewer whose findings drew no verdict at all.
