...
- `atcr verify diff`: scans a unified diff for over-simplification ("reward-hack") fingerprints — `test_only`, `test_deleted`, `test_skipped`, `test_renamed_away`, `weakened_assertion`, `suppression`, `empty_catch`, `stub_body` — and reports a `clean` / `soft_only` / `hard` verdict. Deterministic and model-free: no agent, no provider, no API key, no network. Diff sources, exactly one at a time: `--rev <rev>` (the default, `HEAD`), `--staged`, `--range <a>..<b>`, `--diff <path>`, or `--diff -` for stdin; naming two is a usage error that names both. `--repo` targets a tree other than the CWD. Git is always invoked with `--no-ext-diff --no-color`, so a repo-local `diff.external` or `color.ui = always` cannot feed the parser something that is not a plain unified diff.
- `--fail-on hard|soft|none` on `atcr verify diff` gates the exit code. Gating is **opt-in**: without the flag the verdict is reported and the command exits `0` for every verdict, preserving drop-in parity with the upstream `diff-smell` tool. The standard exit table is reused unchanged — `1` for a tripped gate, `2` for usage errors — so no new exit code is introduced. An invalid value is rejected before any git process is spawned, and a tripped gate still prints the full report. Note the values are diff-smell **verdicts**, not the finding **severities** `atcr review` / `atcr reconcile` / `atcr github` take; the disjoint value sets mean a severity passed here is a usage error rather than a silent misread.
...
- `docs/ci-integration.md`'s authoritative exit table records `atcr verify diff`'s inverted default (0 on every verdict unless `--fail-on`), and `docs/agentic-consumption.md`, `docs/verification.md`, `docs/README.md`, and `README.md` carry matching entries.

### Notes

- **The JSON shape is a public contract from this release.** Committed: the three top-level keys, the smell object's field names, the closed verdict set (`clean`/`soft_only`/`hard`), the closed severity set, and the exit mapping. Adding a new smell **type** is additive and non-breaking — consumers key on `severity` and `summary.verdict`. Renaming a key, removing a field, or changing either vocabulary is **breaking** and will earn a Breaking entry here. `TestSmellResult_GoldenJSON` is the mechanical guard.
