...
- Claims are extracted deterministically from `git log --no-merges` with no model in the path, so the same range always produces byte-identical claims, and the ledger is identical for every agent in a fan-out.
- The claim ledger is exempt from byte-budget shedding: when the budget cannot fund both, diff content is dropped and the ledger is kept.
