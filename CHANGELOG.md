...
- `cli.Hooks`, `cli.ModelInvocationObserver`, `cli.ModelInvocation`, `cli.ModelMessage`, and `cli.ModelToolCall` — the exported hook surface. Every field is a plain type: the intended consumer is a separate Go module (the private `atcr-enterprise` wrapper), for which anything under `internal/` is unreachable by Go's internal-package rule, so no `internal/` type may appear here. A test enforces that structurally.
