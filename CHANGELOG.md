...
- `truncated` on a finding's `verification` block (additive, `omitempty`, same shape as `challenge_survived`). A verdict reached from a shortened read now reaches the artifacts that read verdicts: `report.md` appends `(answered from a truncated read)` — on the main finding line AND in the collapsed Refuted section, which is where it matters most, since refuted is the verdict the gate excludes — and the reviewer's durable `survived_skeptic_rate` leaves such a verdict out of the ratio entirely, neither numerator nor denominator, rather than charging a partial read as a full-confidence one. Previously the fact reached `reconciled/verification.json` alone. **`truncated` is the key the score reads**, i.e. the same artifact `report.md` renders from: `reconciled/verification.json` is a point-in-time verify snapshot that later stages deliberately do not recompute, so a judge ruling that clears the caveat moves the score with it. **When EVERY verdict in a run is truncated the key is OMITTED, not published as `0.0`** — an all-truncated run leaves nothing to divide, and a published `0.0` is indistinguishable from a reviewer whose findings were all refuted, which is the strongest negative signal the metric carries. The same omission covers a reviewer whose findings drew no verdict at all.
