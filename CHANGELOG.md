...
- The promoted merge record is now internally coherent: `resolved_window` and `reserved_output_tokens` come from the same re-packed chunk whose budget was promoted, rather than being left at the first chunk's. Promoting the budget alone wrote a pair no single agent could produce — a 200k-token window beside a 32k backup's budget — and did so order-dependently, since the same re-fit on chunk 0 inherited coherently and on chunk 1 did not.
