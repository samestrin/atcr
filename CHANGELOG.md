...
- Claims are extracted deterministically from `git log --no-merges` with no model in the path, so the same range always produces byte-identical claims, and the ledger is identical for every agent in a fan-out on the normal path. Two exceptions are deliberate and documented in `internal/payload/claims.go`: a `review_strategy: chunked` run puts the ledger in the first chunk only, and an agent whose effective budget is 0 may receive the ledger as its sole entry.
