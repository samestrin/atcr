...
- A silenced span is now judged a fidelity loss on the whole span NAME, not on the fragment the word break left behind, when the break dropped a spaceless-script prefix. Measured: ``the 設定_a() helper drops the error that `retryOnce` returns`` reported the extraction as a clean read while the searchable name `設定_a` had been destroyed, so a co-cited absent anchor could route a REAL finding to the unresolved sidecar and durably charge the reviewer for it. `버퍼_i()` and `解析_x()` are the same shape. A break that drops a SPACING-script prefix (`parse_解`) is unchanged — there the fragment is the right subject.
