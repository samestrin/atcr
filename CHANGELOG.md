...
- Two guard clauses that survived deletion with the whole suite still green are now pinned by tests that fail when either is removed, so a later refactor cannot drop them silently. Both were re-confirmed alive before the tests were written, and both verified dead after.
